...
import { BlocksIcon,BookOpenIcon,BotIcon,CalendarClockIcon,Code2Icon,PlugZapIcon,StoreIcon } from "lucide-react";
...
function VisualLine({ width, opacity = 0.14 }: { width: string; opacity?: number }) {
  return (
    <div
      style={{
        width,
...
export function SandboxVisual({ accent, pulse }: MiniVisualProps) {
  return (
    ...
    >
      ...
      <div style={{ color: COLORS.success, opacity: pulse * 0.3 + 0.7 }}>✓ isolated · complete</div>
      ...
      {["O", "A", "G"].map((label, index) => (
        <div
          ...
          style={{
            ...
            boxShadow: index === 1 ? `0 0 ${18 + pulse * 10}px ${accent}3d` : "none",
            fontSize: 12,
            fontWeight: 800,
          }}
        >
