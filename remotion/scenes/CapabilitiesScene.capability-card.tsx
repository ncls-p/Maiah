    ...
    >
      {[0, 1, 2, 3, 4].map((index) => {
        ...
        return (
          ...
          >
            {active ? <CheckIcon size={12} strokeWidth={2.5} /> : <span style={{ fontSize: 9 }}>0{index + 1}</span>}
          </div>
        );
      })}
    </div>
...
function MiniVisual({ type, accent }: { type: Capability["visual"]; accent: string }) {
  const frame = useCurrentFrame();
  const Visual = MINI_VISUALS[type];
  return <Visual accent={accent} frame={frame} pulse={(Math.sin(frame / 8) + 1) / 2} />;
}
  ...
  return (
    <div
      style={{
        ...
        padding: 24,
        borderRadius: 23,
        background: "linear-gradient(145deg, rgba(18,45,53,0.94), rgba(10,29,35,0.92))",
