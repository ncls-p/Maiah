...
export function SidebarRow({ icon: Icon, label, active = false }: { icon: LucideIcon; label: string; active?: boolean }) {
  return (
    <div
      style={{
        display: "flex",
    ...
    >
      <Icon size={15} strokeWidth={1.8} color={active ? COLORS.azureBright : undefined} />
      <span>{label}</span>
    </div>
  );
}

export function SpecialistRun({ name, detail, icon: Icon, start, color }: { name: string; detail: string; icon: LucideIcon; start: number; color: string }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        >
          {name}
          <span style={{ color: "#748f96", fontSize: 10, fontWeight: 500 }}>pinned · v3</span>
        </div>
        <div
          style={{
            marginTop: 4,
      ...
      <div
        style={{
          ...
          background: completed > 0.5 ? COLORS.success : "rgba(255,255,255,0.04)",
  ...
  return (
    <div
      style={{
        ...
        padding: "15px 17px",
        borderRadius: 16,
        background: "linear-gradient(135deg, rgba(37,173,197,0.11), rgba(15,37,44,0.72))",
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ color: COLORS.white, fontSize: 13, fontWeight: 700 }}>Launch brief ready</div>
            <div style={{ color: "#7e999f", fontSize: 10, marginTop: 2 }}>3 specialists · 12 sources · 4.2k tokens</div>
          </div>
        </div>
        <div
          style={{
        ...
        {[0.93, 0.76, 0.58].map((width, index) => (
          <div
            ...
            style={{
              ...
              height: index === 0 ? 5 : 4,
              borderRadius: 10,
              background: index === 0 ? "rgba(213,235,238,0.5)" : "rgba(143,168,174,0.26)",
            }}
          />
        ))}
      </div>
