...
export function rise(frame: number, start: number, distance = 48, duration = 24): CSSProperties {
...
export function scaleIn(frame: number, start: number, from = 0.9, duration = 24): CSSProperties {
  const value = progress(frame, start, duration);
  return {
    opacity: value,
...
export function sceneOpacity(frame: number, duration: number, options: { fadeIn?: boolean; fadeOut?: boolean; fadeFrames?: number } = {}): number {
