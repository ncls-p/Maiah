  ...
  return (
    <AbsoluteFill style={{ background: COLORS.ink }}>
      <Audio
        ...
        volume={(frame) =>
          interpolate(frame, [0, 45, VIDEO_DURATION - 90, VIDEO_DURATION], [0, 0.72, 0.72, 0], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          })
        }
      ...
      <Sequence from={SCENES.opening.start} durationInFrames={SCENES.opening.duration} premountFor={30}>
        <OpeningScene duration={SCENES.opening.duration} />
      </Sequence>
      <Sequence from={SCENES.chat.start} durationInFrames={SCENES.chat.duration} premountFor={30}>
        <ChatScene duration={SCENES.chat.duration} />
      </Sequence>
      <Sequence from={SCENES.orchestration.start} durationInFrames={SCENES.orchestration.duration} premountFor={30}>
        <OrchestrationScene duration={SCENES.orchestration.duration} />
      </Sequence>
      <Sequence from={SCENES.capabilities.start} durationInFrames={SCENES.capabilities.duration} premountFor={30}>
        <CapabilitiesScene duration={SCENES.capabilities.duration} />
      </Sequence>
      <Sequence from={SCENES.trust.start} durationInFrames={SCENES.trust.duration} premountFor={30}>
        <TrustScene duration={SCENES.trust.duration} />
      </Sequence>
      <Sequence from={SCENES.closing.start} durationInFrames={SCENES.closing.duration} premountFor={30}>
