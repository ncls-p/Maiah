...
describe("conversation sharing", () => {
  it("rejects missing and expired conversations", async () => {
    ...
    dbModule._chain.limit.mockResolvedValueOnce([
      { ...conversation, expiresAt: new Date("2026-08-10T08:59:59.000Z") },
    ]);
  ...
  it("grants owners full shared access", async () => {
    ...
    await expect(
      getConversationAccess("conversation-1", "owner-1"),
  ...
  it("resolves recipient access and rejects users without a share", async () => {
    ...
    await expect(
      getConversationAccess("conversation-1", "recipient-1"),
    ).resolves.toMatchObject({
      role: "recipient",
      canContinue: true,
  ...
  it("creates or updates a share and forces fork mode without continuation", async () => {
    ...
    dbModule._chain.returning.mockResolvedValueOnce([
      { id: "share-1", continuationMode: "fork" },
    ]);
    ...
    expect(dbModule._chain.values).toHaveBeenCalledWith(
      expect.objectContaining({ continuationMode: "fork" }),
    );
    expect(dbModule._chain.onConflictDoUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        set: expect.objectContaining({ continuationMode: "fork" }),
      }),
    );
  });
