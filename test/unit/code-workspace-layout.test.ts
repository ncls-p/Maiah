...
import { codeWorkspaceGridTemplate,DEFAULT_CHAT_WIDTH,DEFAULT_CODE_WORKSPACE_LAYOUT,MAX_CHAT_WIDTH,MIN_CHAT_WIDTH,normalizeCodeWorkspaceChatWidth,normalizeCodeWorkspaceLayout,resizeCodeWorkspacePane,toggleCodeWorkspacePane,visibleCodeWorkspacePanes } from "@/components/chat/code-workspace-layout";
...
describe("code workspace layout", () => {
  ...
  it("keeps every pane independently hideable and restorable", () => {
    const filesHidden = toggleCodeWorkspacePane(DEFAULT_CODE_WORKSPACE_LAYOUT, "files");
    ...
    expect(visibleCodeWorkspacePanes(toggleCodeWorkspacePane(previewHidden, "preview"))).toEqual(["preview"]);
  });

  it("builds responsive desktop columns only for visible panes", () => {
    expect(codeWorkspaceGridTemplate(DEFAULT_CODE_WORKSPACE_LAYOUT)).toBe("208px 0.75rem minmax(240px, 320px) 0.75rem minmax(0, 1fr)");
    expect(
  ...
  it("clamps pointer and keyboard resize values", () => {
    expect(resizeCodeWorkspacePane(DEFAULT_CODE_WORKSPACE_LAYOUT, "files", 999).filesWidth).toBe(360);
    expect(resizeCodeWorkspacePane(DEFAULT_CODE_WORKSPACE_LAYOUT, "code", 10).codeWidth).toBe(240);
