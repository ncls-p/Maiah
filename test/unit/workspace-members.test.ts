...
let matchesPermission: (grantedPermission: string, requiredPermission: string) => boolean;
...
function roleHasPermission(role: { permissions: string[] } | undefined, requiredPermission: string) {
  return Boolean(role?.permissions.some((permission) => matchesPermission(permission, requiredPermission)));
}
...
beforeAll(async () => {
  ...
  ({ matchesPermission } = await import("@/server/domain/services/authorization"));
});
...
describe("workspace IAM", () => {
  ...
  it("keeps app-management permissions for hidden primary workspace access", async () => {
    const { SYSTEM_ROLES } = await import("@/server/domain/entities/iam");
    const adminRole = SYSTEM_ROLES.find((role) => role.name === "workspace.admin");
    const memberRole = SYSTEM_ROLES.find((role) => role.name === "workspace.member");
    const memberPermissions = ["agents.chat", "tools.executeRestricted", "providers.viewMetadata", "models.view", "apiKeys.manageOwn"];
