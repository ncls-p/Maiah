...
describe("access resource registry", () => {
  it("covers every configurable project resource", () => {
    expect(ACCESS_RESOURCE_TYPES).toEqual(
    ...
    expect(ROLE_BINDING_RESOURCE_TYPES).toEqual([
      "organization",
      "workspace",
      ...ACCESS_RESOURCE_TYPES,
    ]);
  ...
  it("keeps one definition and at least one permission domain per type", () => {
    expect(ACCESS_RESOURCE_DEFINITIONS).toHaveLength(
      ACCESS_RESOURCE_TYPES.length,
    );
    for (const type of ACCESS_RESOURCE_TYPES) {
      const definition = ACCESS_RESOURCE_DEFINITIONS.find(
