...
describe("tool approval policy", () => {
  ...
  it("lets an explicit organization policy suppress the default high-risk approval", () => {
    expect(
      decideToolApproval({
  ...
  it("keeps an agent risk policy stricter than the organization default", () => {
    expect(
      decideToolApproval({
        policy: { requireApprovalRiskLevels: ["high"] },
        toolName: "run_code_sandbox",
        toolSource: "builtin",
        riskLevel: "high",
        skipDefaultRiskApproval: true,
