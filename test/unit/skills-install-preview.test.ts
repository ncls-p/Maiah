...
type DbModule = { db: { insert: ReturnType<typeof vi.fn> }; _c: Chain };
vi.mock("@/server/infrastructure/db", () => {
	const chain = makeChain();
	return { db: { insert: vi.fn() }, _c: chain };
});
...
function createInstalledSkillTree(tempDir: string) {
...
describe("skills install and preview", () => {
	...
	it("installs skills into the database and stores sanitized CLI output", async () => {
		const rows = await installSkillsFromCommand({
			workspaceId: "ws-1",
			userId: "user-1",
			installCommand: "npx skills add owner/repo@research-skill",
			isGlobal: true,
		});
