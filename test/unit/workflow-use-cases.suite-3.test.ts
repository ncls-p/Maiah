...
describe("workflow CRUD use cases", () => {
  it("rejects missing, archived, and versionless workflows", async () => {
    database.chain.limit.mockResolvedValueOnce([]);
    await expect(getWorkflowDetail("missing", "workspace-1")).rejects.toBeInstanceOf(WorkflowNotFoundError);

    database.chain.limit.mockResolvedValueOnce([{ ...workflow, status: "archived" }]);
    await expect(getWorkflowDetail("workflow-1", "workspace-1")).rejects.toBeInstanceOf(WorkflowNotFoundError);

    database.chain.limit.mockResolvedValueOnce([workflow]).mockResolvedValueOnce([]);
    await expect(getWorkflowDetail("workflow-1", "workspace-1")).rejects.toBeInstanceOf(WorkflowConflictError);
  });
});
