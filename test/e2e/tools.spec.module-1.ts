...
test.describe("tools hub page", () => {
  ...
  test("switches tabs locally without remounting the controls", async ({ page }) => {
    await page.goto("/en/tools");
    ...
    page.on("request", (request) => {
      ...
      if (url.pathname === "/en/tools" && url.searchParams.get("tab") === "skills") {
        routeRequests += 1;
      }
    });
    ...
    await expect(page).toHaveURL(/\/en\/tools$/);
    await expect(tabList).toHaveAttribute("data-persistence-check", "tools-tabs");
    await expect(page.getByRole("tab", { name: "Skills", exact: true })).toHaveAttribute("data-state", "active");
    await expect(page.getByRole("searchbox", { name: /Search skills/i })).toBeVisible();
    await expect.poll(() => routeRequests).toBe(0);
    await expect(tabList.evaluate((element) => {
      const rect = element.getBoundingClientRect();
      return { height: rect.height, width: rect.width, x: rect.x, y: rect.y };
