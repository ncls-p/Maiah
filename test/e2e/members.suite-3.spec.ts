...
import { e2eMember,ensureE2ELifecycleProject,ensureE2EMember,ensureE2ETransferScenario,ensureE2EUser,login } from "./fixtures";
...
test.describe("members page", () => {
  test("renames and deletes a project from the unified manage menu", async ({ page }) => {
    ...
    await page.getByRole("option", { name: "Lifecycle browser project", exact: true }).click();

    await page.getByRole("button", { name: "Manage", exact: true }).click();
    await page.getByRole("menuitem", { name: "Rename project" }).click();
    const renameDialog = page.getByRole("dialog", { name: "Rename project" });
    await renameDialog.getByLabel("Project name").fill("Lifecycle browser project renamed");
    ...
    await expect(activeProject).toContainText("Lifecycle browser project renamed");
    ...
    await deleteDialog.getByLabel(/Type “Lifecycle browser project renamed” exactly/).fill("Lifecycle browser project renamed");
    ...
    await expect(activeProject).not.toContainText("Lifecycle browser project renamed");
  });

  test("deletes any governed resource from the resource table", async ({ page }) => {
    ...
    await resourceRow.getByRole("button", { name: "Delete Removable assistant" }).click();
    const deleteDialog = page.getByRole("alertdialog");
    await expect(deleteDialog.getByText("Delete Removable assistant?")).toBeVisible();
    await deleteDialog.getByRole("button", { name: "Delete resource" }).click();
    await expect(resourceRow).not.toBeVisible({ timeout: 10_000 });
  });

  test("grants project access through an organization team", async ({ page }) => {
    ...
    await personDialog.getByRole("button", { name: "Add to organization" }).click();
    ...
    const teamCard = page.locator('[data-slot="card"]').filter({ hasText: teamName });
    ...
    await expect(teamCard.locator('[data-slot="badge"]').filter({ hasText: e2eMember.name })).toBeVisible();
    ...
    await page.getByPlaceholder("Search people, email, role, or team…").fill(teamName);
    await expect(page.locator("tbody tr").filter({ hasText: e2eMember.email })).toHaveCount(1);
  });
});
