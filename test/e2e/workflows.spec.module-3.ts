...
test("collects sensitive workflow information without exposing it in chat", async ({ page }) => {
  ...
  try {
    ...
    await page.route(`**/api/workspace/workflows/${workflowId}/agentic/inputs/${requestId}`, async (route) => {
      const body = route.request().postDataJSON() as {
    ...
    const continuation = page.waitForRequest((request) => {
      if (request.method() !== "POST" || !request.url().endsWith(`/workflows/${workflowId}/agentic`)) {
        return false;
      }
      return (request.postDataJSON() as { inputRequestId?: string }).inputRequestId === requestId;
    });
    await page.getByRole("button", { name: "Submit" }).click();
    await continuation;

    await expect(page.getByText("The connection was configured securely.")).toBeVisible();
