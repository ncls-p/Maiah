...
test("switches between visual and agentic editing while keeping live changes", async ({ page }) => {
  ...
  try {
    await page.route(`**/api/workspace/workflows/${workflowId}/agentic`, async (route) => {
    ...
    await expect(page.getByRole("textbox", { name: "Workflow name" })).toHaveValue("Live summary");
    const conditionNode = page.locator('.react-flow__node[data-id="condition"]');
    await expect(conditionNode.locator('.react-flow__handle-right[data-handleid="true"]')).toHaveCount(1);
    await expect(conditionNode.locator('.react-flow__handle-right[data-handleid="false"]')).toHaveCount(1);
    ...
    await expect(page.getByText("Prepare concise summary", { exact: true })).toBeVisible();
    ...
    await page.mouse.move(beforeMove!.x + beforeMove!.width / 2 - 80, beforeMove!.y + beforeMove!.height / 2 + 40, { steps: 6 });
    await page.mouse.up();
    const afterMove = await summaryNode.boundingBox();
    expect(Math.abs(afterMove!.x - beforeMove!.x)).toBeGreaterThan(40);

    const summarySource = summaryNode.locator(".react-flow__handle-right.source");
    const debugTarget = page.locator('.react-flow__node[data-id="debug"]').locator(".react-flow__handle-left.target");
