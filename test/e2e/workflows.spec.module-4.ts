...
test("shows the live checklist, approves an agentic run, and opens debug details", async ({ page }) => {
  ...
  const workflowId = ((await createResponse.json()) as { workflow: { id: string } }).workflow.id;
  ...
  try {
    ...
    await page.route(`**/api/workspace/workflows/${workflowId}/agentic/runs/${requestId}`, async (route) => {
    ...
    await page.route(`**/api/workspace/workflows/${workflowId}/runs?*`, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
    ...
    await page.route(`**/api/workspace/workflow-runs/${runId}?*`, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
    ...
    await expect(todoDock.getByRole("progressbar", { name: "News workflow progress" })).toHaveAttribute("aria-valuenow", "2");
    const agentComposer = page.getByRole("textbox", {
      name: /When a request arrives, have an assistant analyze it/i,
    });
    const [dockBox, composerBox] = await Promise.all([todoDock.boundingBox(), agentComposer.boundingBox()]);
    ...
    await expect(page.getByText("HTTP 401: invalid API key", { exact: true })).toBeVisible();
