...
test.describe("tools hub page", () => {
	...
	test("shows tools tabs", async ({ page }) => {
		...
		await expect(
			page.getByText(/Tools|Built-in|Capabilities/i).first(),
		).toBeVisible({ timeout: 10_000 });
	});
	...
	test("shows built-in tools", async ({ page }) => {
		...
		await expect(
			page.getByText(/Built-in|Platform toolkit|Native capabilities/i).first(),
		).toBeVisible({ timeout: 10_000 });
	});
...
test.describe("custom tools page", () => {
	test("loads custom tools page", async ({ page }) => {
		...
		await expect(page).toHaveURL(/\/en\/custom-tools/);

		await expect(
			page.getByRole("heading", { name: /Custom tools/i }).first(),
		).toBeVisible({ timeout: 10_000 });
	});
	...
	test("shows custom tools empty state", async ({ page }) => {
		...
		await expect(
			page.getByText(/Custom tools|No custom tools|Create/i).first(),
		).toBeVisible({ timeout: 10_000 });
	});
});
