...
import { agents,aiModels,aiProviders,roleBindings,workspaces } from "@/server/infrastructure/db/schema";
import type { IamDatabaseScenarioContext } from "./iam-use-cases-db.context";

export async function runIamDatabaseScenario1(context: IamDatabaseScenarioContext) {
  const { suffix, ownerId, memberId, organizationIds, memberEmail } = context;
  let { organizationId, firstProjectId, secondProjectId, sharedAgentId } = context;
  ...
  Object.assign(context, { organizationId, firstProjectId, secondProjectId, sharedAgentId });
}
