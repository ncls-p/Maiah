...
import { type ToolRiskLevel } from "@/modules/tool/builtin-tools-catalog";
...
export function BuiltinToolsPanel({ workspaceId, canManage }: { workspaceId: string; canManage: boolean }) {
  ...
  const loadTools = useCallback(async () => {
    ...
    setLoadError(false);
    try {
      const response = await fetch(`/api/workspace/tools?workspaceId=${workspaceId}`);
  ...
  async function updatePolicy(tool: BuiltInToolPolicy, patch: Pick<Partial<BuiltInToolPolicy>, "enabled" | "requireApproval">) {
    const previous = tool;
    setPendingToolNames((current) => new Set(current).add(tool.name));
    setBuiltinTools((current) => current.map((candidate) => (candidate.name === tool.name ? { ...candidate, ...patch } : candidate)));
    try {
      ...
      setBuiltinTools((current) => current.map((candidate) => (candidate.name === updated.name ? updated : candidate)));
      toast.success(t("updateSuccess", { name: tool.displayName }));
    } catch {
      setBuiltinTools((current) => current.map((candidate) => (candidate.name === previous.name ? previous : candidate)));
  ...
  const stats = useMemo(() => {
    return {
      ...
      approval: builtinTools.filter((tool) => tool.enabled && tool.requireApproval).length,
  ...
  const filteredTools = useMemo(() => {
    ...
    return builtinTools.filter((tool) => {
      ...
      return tool.displayName.toLowerCase().includes(query) || tool.name.toLowerCase().includes(query) || tool.description.toLowerCase().includes(query) || tool.category.toLowerCase().includes(query);
    });
  ...
  if (loadError) {
    return (
      <div className="rounded-2xl border border-destructive/25 bg-destructive/5 p-5">
        ...
        <Button type="button" variant="outline" size="sm" className="mt-4" onClick={() => void loadTools()}>
          {t("retry")}
        </Button>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-5 animate-in-fade">
      <section className="rounded-2xl border border-border/70 bg-card/55 p-2.5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="relative min-w-0 flex-1">
            <SearchIcon className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground/70" aria-hidden="true" />
            <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("searchPlaceholder")} className="h-10 pl-9" aria-label={t("searchPlaceholder")} />
          </div>
          <div className="flex shrink-0 items-center gap-1.5 text-[0.68rem] text-muted-foreground">
            ...
            <span className="rounded-full bg-muted px-2 py-1">{t("resultsCount", { count: filteredTools.length })}</span>
          </div>
        </div>
        <div className="mt-2 flex flex-nowrap gap-1.5 overflow-x-auto pb-0.5">
          <Button type="button" size="sm" variant={categoryFilter === "all" ? "default" : "outline"} className="h-8 shrink-0 rounded-full px-3 text-xs" onClick={() => setCategoryFilter("all")}>
            {t("allCategories")}
          </Button>
          {categories.map((category) => {
            ...
            return (
              <Button key={category} type="button" size="sm" variant="outline" className={cn("h-8 shrink-0 gap-1.5 rounded-full border px-3 text-xs", active ? "border-input bg-muted text-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground")} onClick={() => setCategoryFilter(category)}>
      ...
      {filteredTools.length === 0 ? (
        <PageEmptyState icon={SearchIcon} title={t("noResults")} description={t("noResultsHint")} />
      ) : (
          ...
          {filteredTools.map((tool) => (
            <li key={tool.id}>
              <BuiltinToolCard
                ...
                onEnabledChange={(enabled) => void updatePolicy(tool, { enabled })}
                onApprovalChange={(requireApproval) => void updatePolicy(tool, { requireApproval })}
