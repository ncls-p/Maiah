...
export type KnowledgePageViewModel = Extract<ReturnType<typeof useKnowledgePageController>, { kind: "ready" }>;
export function KnowledgePageView({ model }: { model: KnowledgePageViewModel }) {
  const { baseForm, bases, canManageKnowledgeBases, canManageTenantGlobals, createBase, defaultRagConfig, deleteBase, deleteDocument, deleting, loading, pendingDelete, setBaseForm, setPendingDelete, setShowCreateDialog, showCreateDialog, t, tCommon } = model;
  return (
    <WorkspacePage title={t("orbitTitle")} accentTitle={t("orbitAccent")} eyebrow={t("orbitEyebrow")} description={t("orbitDescription")} width="wide" actions={canManageKnowledgeBases && !loading && bases.length > 0 ? <KnowledgePageBranch6 model={model} /> : null}>
      <Dialog open={canManageKnowledgeBases && showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="max-h-[calc(100svh-2rem)] overflow-y-auto">
          ...
          <div className="grid gap-3">
            ...
            {canManageTenantGlobals ? <KnowledgePageBranch5 model={model} /> : null}
            <AdvancedSection label={t("ragAdvanced")} hint={t("ragCreateAdvancedHint")} storageKey="advanced:knowledge-create-rag-config">
              <div className="grid gap-4">
                ...
                {baseForm.customizeRag ? <KnowledgePageBranch4 model={model} /> : null}
              </div>
            </AdvancedSection>
          </div>
          <DialogFooter>
      ...
      {loading ? <KnowledgePageBranch3 model={model} /> : bases.length === 0 ? <KnowledgePageBranch2 model={model} /> : <KnowledgePageBranch1 model={model} />}
      <AlertDialog
