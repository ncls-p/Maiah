...
export function ChatPageView({ model }: { model: Model }) {
  ...
  const { deleteMessage, editMessage, emptyPromptSuggestions, ephemeral, input, interfaceMode, latestTodoList, loadingMessages, messages, organizationDefaultAgentId, pendingApprovals } = model;
  const { queuedMessages, quota, rejectToolInvocation, reloadActualLatestMessages, reloadAgentContext, resendMessage, selectAgent, selectedAgent, selectedAgentId, sending, setAttachments, setEphemeral } = model;
