...
import type { ChatConversation,ChatMessage,CodeWorkspaceArtifact } from "@/components/chat/chat-types";
...
export { latestConversationIdFromList,normalizeConversationList,type ConversationListPage,type ConversationListPayload } from "./chat-conversation-list";

export function mergeConversationPages(current: ChatConversation[], incoming: ChatConversation[]) {
  const existingIds = new Set(current.map((conversation) => conversation.id));
  return [...current, ...incoming.filter((conversation) => !existingIds.has(conversation.id))];
}
...
export function rotatePromptSuggestions(suggestions: string[], seed: string) {
  ...
  const offset = Array.from(seed).reduce((total, char) => total + char.charCodeAt(0), 0) % suggestions.length;
  return [...suggestions.slice(offset), ...suggestions.slice(0, offset)].slice(0, 3);
}

export function upsertConversation(current: ChatConversation[], conversation: ChatConversation) {
  let found = false;
...
export function latestCodeWorkspaceArtifact(messages: ChatMessage[]) {
  ...
  for (const message of messages) {
    for (const part of message.parts) {
      if (part.type !== "file" && part.type !== "tool-call" && part.type !== "tool-result") {
        continue;
      }
...
export function ChatContextBar({ quota }: { quota: { used: number; limit: number } | null }) {
  ...
  return (
    <Collapsible open={open} onOpenChange={updateOpen} className="shrink-0 border-b border-border/60 bg-background">
      <div className="mx-auto flex min-h-10 w-full max-w-4xl items-center justify-between gap-3 px-4 py-1.5">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          <span className="truncate text-sm font-medium">{t("statusTitle")}</span>
          {showQuota ? (
            <Badge variant={quotaPercent >= 100 ? "destructive" : "outline"} className="rounded-lg text-[11px] font-medium">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <CollapsibleTrigger asChild>
            <Button type="button" size="sm" variant="ghost" className="h-7 gap-1 px-2 text-xs" aria-label={open ? t("hideContext") : t("showContext")}>
              <ChevronDownIcon className={cn("size-3 transition-transform", !open && "-rotate-90")} aria-hidden="true" />
            </Button>
          </CollapsibleTrigger>
        </div>
      </div>
      {showQuota ? (
        <CollapsibleContent>
          <div className="flex flex-col gap-0">{quota ? <QuotaBanner used={quota.used} limit={quota.limit} /> : null}</div>
        </CollapsibleContent>
      ) : null}
    </Collapsible>
  );
