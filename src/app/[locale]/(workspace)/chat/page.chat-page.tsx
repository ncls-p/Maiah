...
export default function ChatPage() {
  ...
  const fetchConversationPage = useCallback(
    async ({
      ...
      const data = await fetchJson<ConversationListPayload>(`/api/workspace/conversations?${params.toString()}`, { signal });
  ...
  const loadAgentDirectory = useCallback(
    async ({
      ...
      const response = await fetchJson<AgentDirectoryPayload | ChatAgent[]>(`/api/workspace/agents?${agentParams.toString()}`, { signal });
      ...
      setAgents(data);
      setOrganizationDefaultAgentId(responseDefaults.organizationDefaultAgentId ?? null);
      setCanCreateAgent(Boolean(responseDefaults.canCreateAgent));
      setCanRunSetup(Boolean(responseDefaults.canCreateAgent && responseDefaults.canManageProviders));
      ...
      const nextAgentId = (requestedAgentId && data.some((agent) => agent.id === requestedAgentId) ? requestedAgentId : null) ?? (preferredAgentId && data.some((agent) => agent.id === preferredAgentId) ? preferredAgentId : null) ?? (responseDefaults.effectiveDefaultAgentId && data.some((agent) => agent.id === responseDefaults.effectiveDefaultAgentId) ? responseDefaults.effectiveDefaultAgentId : null) ?? data[0]?.id ?? null;
  ...
  const loadMoreConversations = useCallback(async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t("errors.loadOlderFailed"));
      return;
    } finally {
      setLoadingMoreConversations(false);
    }
  }, [conversationCursor, conversations, fetchConversationPage, hasMoreConversations, loadingMoreConversations, t]);
  ...
  const loadMoreConversationSearchResults = useCallback(async () => {
    const query = conversationSearchQuery.trim();
    if (!query || conversationSearchState.query !== query || !conversationSearchState.hasMore || conversationSearchState.loadingMore || !conversationSearchState.nextCursor) {
      return;
    }
  ...
  const { messages, setMessages, sending, pendingApprovals, handleSubmit, resolveApproval, stopGeneration, detachActiveStream } = useChatStream({
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      setCodeWorkspaceArtifact((current) => {
        if (current?.projectId === latestArtifact.projectId && latestArtifact.version <= current.version) {
          return current;
        }
  ...
  useEffect(() => {
    if (sending || !canChat || queuedMessages.length === 0 || processingQueuedMessageRef.current) {
      return;
    }
    ...
    queueMicrotask(() => {
      setQueuedMessages((current) => (current[0]?.id === nextMessage.id ? current.slice(1) : current.filter((message) => message.id !== nextMessage.id)));
  ...
  }, [canChat, codeWorkspaceArtifact, handleSubmit, interfaceMode, queuedMessages, sending]);
  ...
  useEffect(() => {
    ...
    async function loadMessages() {
      try {
        ...
        if (cancelled) return;
        const urlAgentId = new URL(window.location.href).searchParams.get("agentId");
        ...
        if (loadedConversation) {
          setConversations((current) => upsertConversation(current, loadedConversation));
        }
  ...
  async function deleteConversation(conversationId: string) {
    ...
    try {
      ...
      if (activeConversationId === conversationId) {
        ...
        resetInterfaceMode();
        window.history.replaceState(null, "", selectedAgentId ? `/chat?agentId=${selectedAgentId}` : "/chat");
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("errors.deleteConversationFailed"));
      return;
    } finally {
      setDeleting(false);
    }
  ...
  async function renameConversationFolder(folderId: string, name: string) {
    try {
      ...
      setConversationFolders((current) => current.map((folder) => (folder.id === folderId ? data.folder : folder)));
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("errors.renameFolderFailed"));
      return;
    }
  }
  ...
  async function deleteConversationFolder(folderId: string) {
    ...
    try {
      ...
      setConversationFolders((current) => current.filter((folder) => folder.id !== folderId));
      setConversations((current) => current.map((conversation) => (conversation.folderId === folderId ? { ...conversation, folderId: null } : conversation)));
      setPendingDelete(null);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("errors.deleteFolderFailed"));
      return;
    } finally {
      setDeleting(false);
    }
  ...
  async function toggleConversationPin(conversationId: string, isPinned: boolean) {
    try {
      ...
      setConversations((current) => current.map((conversation) => (conversation.id === conversationId ? { ...conversation, ...data.conversation } : conversation)));
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("errors.updatePinFailed"));
      return;
    }
  }
  ...
  function skipPendingSuggestions() {
    ...
    void fetch(`/api/workspace/conversations/${activeConversationId}/skip-suggestions`, { method: "POST" }).catch(() => undefined);
  }
  ...
  function submitMessage() {
    ...
    const content = input.trim() || (hasAttachments ? (attachments.every((attachment) => attachment.kind === "chat_image") ? t("attachments.analyzeImage") : t("attachments.analyzeFile")) : "");
  ...
  async function uploadCodeWorkspace(files: File[]) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t("attachments.codeUploadFailed"));
      return;
    }
  }
  ...
  async function uploadChatAttachment(file: File) {
    ...
    try {
      ...
      toast.success(data.attachment.kind === "chat_image" ? t("attachments.imageAttached") : t("attachments.fileAttached"));
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("attachments.uploadFailed"));
      return;
    }
  }
  ...
  async function setUserDefaultAgent(agentId: string | null) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t("defaultSaveFailed"));
      return;
    }
  }

  function updateQueuedMessage(id: string, content: string) {
    setQueuedMessages((current) => current.map((message) => (message.id === id ? { ...message, content } : message)));
  }
  ...
  async function editMessage(message: ChatMessage, content: string) {
    if (!activeConversationId) return;
    const trimmedContent = content.trim();
    await fetchJson(`/api/workspace/conversations/${activeConversationId}/messages/${message.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      ...
      messages.map((item) =>
        item.id === message.id
          ? {
              ...item,
              status: "completed",
              parts: [{ type: "text", content: trimmedContent }, ...item.parts.filter((part) => part.type === "file")],
            }
          : item,
      ),
    );
  ...
  async function deleteMessage(message: ChatMessage) {
    if (!activeConversationId) return;
    await fetchJson(`/api/workspace/conversations/${activeConversationId}/messages/${message.id}`, { method: "DELETE" });
  ...
  async function reloadActualLatestMessages() {
    if (!activeConversationId || sending) return;
    const data = await fetchJson<{ messages?: ChatMessage[] }>(`/api/workspace/conversations/${activeConversationId}`);
  ...
  async function reloadAgentContext() {
    ...
    try {
      ...
      const versionData = await fetchJson<AgentVersion[]>(`/api/workspace/agents/${refreshedAgentId}/versions?workspaceId=${workspaceId}`);
      setActiveVersion(versionData.find((version) => version.isActive) ?? null);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("errors.reloadAgentFailed"));
      return;
    } finally {
    ...
    return (
      <>
        <ChatLayout
          ...
          onSetupComplete={() => void reloadAgentContext()}
        >
          <NoAssistantsState canCreateAgent={canCreateAgent} canRunSetup={canRunSetup} t={t} />
        </ChatLayout>
        {destructiveDialog}
      </>
    );
