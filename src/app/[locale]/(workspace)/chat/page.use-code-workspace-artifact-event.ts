...
export function useCodeWorkspaceArtifactEvent({
  ...
  setCodeWorkspaceArtifact: Dispatch<SetStateAction<CodeWorkspaceArtifact | null>>;
  setInterfaceMode: Dispatch<SetStateAction<InterfaceMode>>;
}) {
  useEffect(() => {
    function handleCodeWorkspaceArtifact(event: Event) {
      const detail = (event as CustomEvent<{ artifact?: CodeWorkspaceArtifact; activate?: boolean }>).detail;
      const artifact = detail?.artifact;
      if (!artifact?.projectId) return;
      setCodeWorkspaceArtifact((current) => {
        if (current?.projectId === artifact.projectId && artifact.version <= current.version) return current;
        return artifact;
      });
    ...
    return () => window.removeEventListener(CODE_WORKSPACE_ARTIFACT_EVENT, handleCodeWorkspaceArtifact);
  }, [lastAutoOpenedWorkspaceRef, setCodeWorkspaceArtifact, setInterfaceMode, userSelectedInterfaceModeRef]);
}
