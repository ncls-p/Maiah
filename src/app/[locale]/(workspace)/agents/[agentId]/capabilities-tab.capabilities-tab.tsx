...
} from "./capabilities-tab.catalog-controls";
...
export function CapabilitiesTab(props: Props) {
  const {
    ...
    setSelectedKnowledgeIdsAction,
    skills,
    selectedSkillIds,
    setSelectedSkillIdsAction,
    saving,
    readOnly = false,
  ...
  const [displayMode, setDisplayMode] = useState<CapabilityDisplayMode>(() => {
    ...
    return window.localStorage.getItem("agent-capabilities:display-mode") ===
      "grid"
      ? "grid"
      : "list";
  });
  ...
  const packages = buildBuiltinToolPackages(builtinTools).filter((item) =>
    matches(
      t(`toolPackages.${item.id}.title`),
      t(`toolPackages.${item.id}.description`),
  ...
  const custom = customTools.filter((tool) =>
  ...
  const show = (value: Exclude<CapabilityFilter, "all">) =>
    filter === "all" || filter === value;
  const visibleCount =
    (show("tools") ? packages.length + custom.length : 0) +
    (show("mcp") ? servers.length : 0) +
    (show("knowledge") ? knowledge.length : 0) +
    (show("skills") ? filteredSkills.length : 0);

  const selectedCustomIds = Object.entries(customBindings)
    .filter(([, binding]) => binding.enabled)
    .map(([id]) => id);
  const setSelectedCustomIds = (update: (current: string[]) => string[]) =>
    setCustomBindingsAction((current) => {
      ...
      return Object.fromEntries(
        customTools.map((tool) => [
          tool.id,
          {
            enabled: selected.has(tool.id),
            requireApproval: current[tool.id]?.requireApproval ?? true,
          },
        ]),
      );
    });
    ...
    >
      <CapabilityCatalogControls
        ...
        counts={{
          all:
            ...
            skills.length,
          tools: builtinTools.length + customTools.length,
          mcp: mcpTools.length,
        ...
        labels={{
          all: tCap("filterAll"),
          tools: tCap("filterTools"),
          mcp: tCap("filterMcp"),
          knowledge: tCap("filterKnowledge"),
          skills: tCap("filterSkills"),
          search: tCap("searchPlaceholder"),
          grid: tList("showAsGrid"),
          list: tList("showAsList"),
        }}
      >
        {visibleCount === 0 ? (
          <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-muted-foreground">
            {tCap("noSearchResults", { query })}
          </div>
        ) : null}
        ...
        {show("tools") && packages.length > 0 ? (
          <ConfigSection
            ...
            icon={WrenchIcon}
            className="bg-background/55 shadow-none"
            stagger="3"
          >
            ...
            <div
              className={cn(
                "grid gap-3",
                displayMode === "grid" && "xl:grid-cols-2",
              )}
            >
              {packages.map((toolPackage) => (
                <BuiltinToolPackageCard
                  ...
                  setBindings={setBuiltinBindingsAction}
                  ...
                  countLabel={t("toolPackageCount", {
                    enabled: toolPackage.tools.filter(
        ...
        {show("tools") && custom.length > 0 ? (
          <ConfigSection
            ...
            stagger="4"
          >
            <CapabilityResourceGrid
              resources={custom}
              selectedIds={selectedCustomIds}
              setSelectedIds={setSelectedCustomIds}
              icon={WrenchIcon}
              displayMode={displayMode}
              toggleLabel={(name) => tCap("toggleTool", { name })}
              labels={{
                active: tCap("active"),
                selected: tCap("selectedCount", {
                  selected: selectedCustomIds.filter((id) =>
                    custom.some((item) => item.id === id),
                  ).length,
                  total: custom.length,
                }),
                enableAll: tCap("enableVisible"),
                disableAll: tCap("disableVisible"),
              }}
            />
          </ConfigSection>
        ) : null}
            ...
            >
              {servers.map((server) => (
                <McpServerCollapsible
                  ...
                  setMcpBindings={setMcpBindingsAction}
        ...
        {show("skills") && filteredSkills.length > 0 ? (
          ...
          >
            <CapabilityResourceGrid
              ...
              setSelectedIds={setSelectedSkillIdsAction}
              icon={BookMarkedIcon}
              displayMode={displayMode}
              toggleLabel={(name) => tCap("toggleSkill", { name })}
              labels={{
                active: tCap("active"),
                selected: tCap("selectedCount", {
                  selected: selectedSkillIds.filter((id) =>
                    filteredSkills.some((item) => item.id === id),
                  ).length,
        ...
        {show("knowledge") && knowledge.length > 0 ? (
          ...
          >
            <CapabilityResourceGrid
              ...
              setSelectedIds={setSelectedKnowledgeIdsAction}
              icon={BookOpenIcon}
              displayMode={displayMode}
              toggleLabel={(name) => tCap("toggleKnowledge", { name })}
              labels={{
                active: tCap("active"),
                selected: tCap("selectedCount", {
                  selected: selectedKnowledgeIds.filter((id) =>
      ...
      {readOnly ? null : (
        <div className="flex justify-end rounded-2xl border border-border/60 bg-card/75 p-3 shadow-[var(--surface-shadow)]">
          <Button type="button" disabled={saving} onClick={onSaveAction}>
            {saving ? (
