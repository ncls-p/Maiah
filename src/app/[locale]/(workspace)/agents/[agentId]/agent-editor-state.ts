 ...
 */
export function mergeAgentEditorState(current: Agent, persisted: Agent, overrides: Partial<Pick<Agent, "shareTargetEmail">> = {}): Agent {
  return {
    ...persisted,
    canAdminCurate: current.canAdminCurate,
    canEdit: current.canEdit,
    canClone: current.canClone,
    shareTargetEmail: Object.hasOwn(overrides, "shareTargetEmail") ? overrides.shareTargetEmail : (persisted.shareTargetEmail ?? current.shareTargetEmail),
  };
}
