...
const MINIMUM_VISIBLE_BAR_PX = 4;

export function usageBarHeight(tokens: number, maxTokens: number) {
  if (tokens <= 0 || maxTokens <= 0) return 0;
  return Math.max(
    MINIMUM_VISIBLE_BAR_PX,
    Math.round((tokens / maxTokens) * CHART_HEIGHT_PX),
  );
}
    ...
    <section className="rounded-2xl border bg-card p-5 shadow-[var(--surface-shadow)]">
      ...
      {points.length === 0 ? (
        ...
        <div
          ...
          aria-label={props.t("dailyTrend")}
        >
          {points.map((point) => {
            const inputHeight = usageBarHeight(point.inputTokens, max);
            const outputHeight = usageBarHeight(point.outputTokens, max);
            return (
              <div
                key={point.date}
                className="group flex h-full min-w-0 flex-1 items-end gap-px"
                title={`${point.date} · ${formatFullCount(point.inputTokens + point.outputTokens, props.locale)}`}
              >
                <div
                  className="flex-1 rounded-t-sm bg-chart-1 transition-opacity group-hover:opacity-75"
                  style={{ height: inputHeight }}
                />
                <div
                  className="flex-1 rounded-t-sm bg-chart-2 transition-opacity group-hover:opacity-75"
