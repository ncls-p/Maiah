...
function QuotaHero({
	...
	return (
		<section className="surface-panel animate-in-up overflow-hidden p-0">
			<div className="relative border-b border-border/60 bg-gradient-to-br from-primary/8 via-background to-chart-2/10 px-5 py-6 sm:px-6 sm:py-7">
...
function UsageFilters({
	...
	const hasFilters = Boolean(
		operationFilter.trim() || fromDate || toDate,
	);

	return (
		<section className="surface-panel animate-in-up stagger-2 p-4 sm:p-5">
			...
			<div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_10rem_10rem_auto] lg:items-end">
				...
				<div className="flex flex-wrap gap-2 lg:justify-end">
					<Button variant="outline" disabled={!hasFilters || busy} onClick={onReset}>
...
function TokenChart({
	...
	return (
		<section className="surface-panel animate-in-up stagger-3 flex h-full flex-col p-5">
			...
			<div className="flex min-w-0 flex-1 flex-col gap-2">
				...
				<div className="flex items-center gap-3 text-sm">
					<span className="inline-flex items-center gap-1 text-muted-foreground">
						<ArrowDownToLineIcon className="size-3.5 text-chart-1" aria-hidden="true" />
						{formatCount(input)}
					</span>
					<span className="inline-flex items-center gap-1 text-muted-foreground">
						<ArrowUpFromLineIcon className="size-3.5 text-chart-2" aria-hidden="true" />
						{formatCount(output)}
					</span>
					<span className="inline-flex items-center gap-1 text-muted-foreground">
...
function UsageEventList({
	...
	return (
		<section className="surface-panel animate-in-up stagger-4 flex h-full flex-col p-5">
			<div className="mb-5 flex flex-col gap-1">
				<div className="flex items-center gap-2">
					<ActivityIcon className="size-4 text-primary" aria-hidden="true" />
					<h2 className="text-base font-semibold">{t("recentUsage")}</h2>
				</div>
				<p className="text-sm text-muted-foreground">{t("recentUsageDescription")}</p>
			</div>
