...
export async function POST(req: NextRequest, { params }: { params: Promise<{ itemId: string }> }) {
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      const forbidden = await requireWorkspacePermissionAsync(session.user.id, parsed.data.workspaceId, "marketplaceItems.install");
    ...
    },
    {
      ...
      expectedError: (error) => {
        ...
        const isNotFound = error instanceof Error && (error.message.includes("not found") || error.message.includes("available"));
