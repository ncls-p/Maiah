...
import { getRegistrationSetting,setRegistrationEnabled } from "@/modules/admin/use-cases";
...
export async function PATCH(req: NextRequest) {
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      const setting = await setRegistrationEnabled(parsed.data.registrationEnabled, session.user.id);
