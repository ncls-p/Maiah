...
import {
  canViewCustomTool,
  canViewMcpServer,
} from "@/modules/tool/use-cases.tool-binding-input-schema";
...
export async function listAvailableCapabilities(input: {
  ...
  const [customRows, mcpRows, skillRows, knowledgeRows] = await Promise.all([
    ...
    db
      .select({
        ...
        serverRequireApproval: mcpServers.requireApproval,
  ...
  const visibleCustom = (
    await Promise.all(
      customRows.map(async (tool) =>
        (await canViewCustomTool(tool, userId)) ? tool : null,
      ),
    )
  ).filter(
    (tool): tool is NonNullable<typeof tool> =>
      tool !== null && tool.status === "active",
  );
  const visibleMcp = (
    await Promise.all(
      mcpRows.map(async (tool) =>
  ...
  const builtin = listBuiltInTools().map((tool) => ({
    id: tool.id,
    ...
    group: tool.category,
    requireApproval:
      approvalByKey.get(`builtin:${tool.id}`) ??
      requiresApproval(tool.riskLevel),
    attached: boundToolKeys.has(`builtin:${tool.id}`),
  }));
  const custom = visibleCustom.map((tool) => ({
    id: tool.id,
    source: "custom" as const,
    name: tool.name,
    description: tool.description,
    group: null,
    requireApproval: approvalByKey.get(`custom:${tool.id}`) ?? true,
    attached: boundToolKeys.has(`custom:${tool.id}`),
  }));
  const mcp = visibleMcp.map((tool) => ({
    id: tool.id,
    source: "mcp" as const,
    name: tool.name,
    description: tool.description,
    ...
    requireApproval:
      approvalByKey.get(`mcp:${tool.id}`) ??
      (tool.serverRequireApproval || tool.requireApproval),
    attached: boundToolKeys.has(`mcp:${tool.id}`),
  }));
  const skills = skillRows.map((skill) => ({
    id: skill.id,
    name: skill.name,
    description: skill.description,
    attached: boundSkillIds.has(skill.id),
  }));
