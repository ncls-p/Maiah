...
import { canEditAgent,getAgentDefaultPreferences,listAgents,normalizePromptSuggestions } from "@/modules/agent/use-cases";
...
export async function GET(req: NextRequest) {
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      const scopeForbidden = await requireRequestPermissionScopeAsync(session.user.id, workspaceId, "agents.list");
      ...
      const [canCreateAgent, canUpdateAgents, canManageProviderSettings, canManageModels] = await Promise.all([hasWorkspacePermissionForRequest(session.user.id, workspaceId, "agents.create"), hasWorkspacePermissionForRequest(session.user.id, workspaceId, "agents.update"), hasWorkspacePermissionForRequest(session.user.id, workspaceId, "providers.update"), hasWorkspacePermissionForRequest(session.user.id, workspaceId, "models.manage")]);
      ...
      const defaultPreferences = await getAgentDefaultPreferences(workspaceId, session.user.id, new Set(list.map((agent) => agent.id)));
      const modelMetaByVersionId = includeModelMeta ? await getModelMetaByVersionId(list.map((agent) => agent.activeVersionId).filter(Boolean)) : new Map<string, { displayName: string | null; logoUrl: string | null }>();
      ...
      const directlyEditableAgentIds = await authorization.listDirectlyAuthorizedResourceIds(
      ...
      const agentsWithAccess = await Promise.all(
        list.map(async (agent) => ({
          ...
          ...(agent.activeVersionId
            ? {
                modelDisplayName: modelMetaByVersionId.get(agent.activeVersionId)?.displayName,
            ...
            : {}),
          canEdit: (canUpdateAgents && canEditAgent(agent, session.user.id, canAdminCurate)) || directlyEditableAgentIds.has(agent.id),
          canClone: canCreateAgent,
        })),
      );
      const agentsWithProvenance = await withResourceProvenance(agentsWithAccess, workspaceId, session.user.id);
