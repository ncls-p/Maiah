...
async function applyConversationSidebarOrder(input: { conversationIds: string[]; folderId: string | null; pinned?: boolean }) {
  try {
...
export async function POST(req: NextRequest) {
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      const forbidden = await requireWorkspacePermissionAsync(session.user.id, workspaceId, "conversations.viewOwn");
      ...
      if (folderId) {
        const [folder] = await db
          ...
          .where(and(eq(conversationFolders.id, folderId), eq(conversationFolders.workspaceId, workspaceId), eq(conversationFolders.userId, session.user.id), isNull(conversationFolders.archivedAt)))
          .limit(1);
        if (!folder) {
      ...
      if (uniqueIds.length !== conversationIds.length) {
        return NextResponse.json({ error: "Duplicate conversations" }, { status: 400 });
      }

      const ownedRows = await db
        .select({ id: conversations.id })
        .from(conversations)
        .where(and(inArray(conversations.id, conversationIds), eq(conversations.workspaceId, workspaceId), eq(conversations.userId, session.user.id), eq(conversations.status, "active"), isNull(conversations.archivedAt)));
