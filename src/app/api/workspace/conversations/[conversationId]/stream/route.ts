...
import { createChatStreamResponse,hasActiveChatStream } from "@/modules/chat/stream-bus";
  ...
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      if (!hasActiveChatStream(streamingMessage.id)) {
        return NextResponse.json({ error: "Stream is no longer active" }, { status: 409 });
      }

      return createChatStreamResponse(streamingMessage.id, { "X-Message-Id": streamingMessage.id }, { replay: true });
    },
