...
import { createKnowledgeBase,listKnowledgeBases,RagModelConfigurationPermissionError } from "@/modules/knowledge/use-cases";
...
export async function GET(req: NextRequest) {
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      const knowledgeBases = await listKnowledgeBases(parsed.data.workspaceId, session.user.id, canManageGlobal);
      return NextResponse.json(await withResourceProvenance(knowledgeBases, parsed.data.workspaceId, session.user.id));
    },
...
export async function POST(req: NextRequest) {
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      const forbidden = await requireWorkspacePermissionAsync(session.user.id, parsed.data.workspaceId, "knowledgeBases.manage");
      if (forbidden) return forbidden;
      const canManageGlobal = await canManageTenantGlobals(session, parsed.data.workspaceId);
      ...
      const [knowledgeBaseWithProvenance] = await withResourceProvenance([knowledgeBase], parsed.data.workspaceId, session.user.id);
      return NextResponse.json(knowledgeBaseWithProvenance, { status: 201 });
    },
    {
      logLabel: "Failed to create knowledge base",
      expectedError: (error) => (error instanceof RagModelConfigurationPermissionError ? NextResponse.json({ error: error.message }, { status: 403 }) : NextResponse.json({ error: "Failed to create knowledge base" }, { status: 500 })),
    },
  );
}
