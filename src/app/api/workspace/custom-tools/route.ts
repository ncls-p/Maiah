...
export async function GET(req: NextRequest) {
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      const scopeForbidden = await requireRequestPermissionScopeAsync(session.user.id, parsed.data.workspaceId, "tools.view");
      ...
      const tools = await listCustomTools(parsed.data.workspaceId, session.user.id, canManageGlobal);
      return NextResponse.json(await withResourceProvenance(tools, parsed.data.workspaceId, session.user.id));
    },
