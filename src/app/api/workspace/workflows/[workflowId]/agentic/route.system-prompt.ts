...
export function createWorkflowAgentSystemPrompt(input: {
  ...
  return [
    ...
    "Use workflow.run to chain published workflows discovered with list_workflows. Parameters: workflowId, input (defaults to {{input}}), outputPath (defaults to workflowResult). The result contains runId and output indexed by child step ID. Set maxRetries: 0. Calls wait for the child, propagate errors, and reject cycles; a chain is limited to five levels and twenty child runs.",
    "Use tool.call nodes for direct tool execution without an assistant. Call list_workflow_tools to discover real tool IDs and argument schemas. Use source, toolId, arguments and outputPath parameters. Values such as {{field.path}} preserve JSON types when they occupy the full value. Direct tool calls must have maxRetries: 0 to avoid duplicate external actions. The selected assistant also has its configured tools, knowledge search and skills available; use them when relevant to designing the workflow.",
