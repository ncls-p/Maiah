...
import { getActiveVersion,getAgentById,getAgentDefaultPreferences,listAgents,resolveProviderForVersion } from "@/modules/agent/use-cases";
import { chatTodoListInputSchema,type ChatTodoList } from "@/modules/chat/todo-list";
import { codeSandboxInputSchema,searchWebWithSearxng,webSearchInputSchema } from "@/modules/tool/builtin-tool-primitives";
import { executeCodeSandbox } from "@/modules/tool/code-sandbox";
import { validateWorkflowAgentDraft,workflowAgentCatalogPrompt,workflowAgenticRequestSchema,workflowAgentPromptDraft,workflowAgentToolLabels,type WorkflowAgenticDraft } from "@/modules/workflows/agentic";
import { appendWorkflowAgentMessage,consumeWorkflowAgentInputRequest,createWorkflowAgentInputRequest,getWorkflowAgentHistory,workflowAgentInputFieldSchema,type WorkflowAgentInputRequest } from "@/modules/workflows/agentic-history";
...
import { workflowDefinitionSchema,workflowEdgeSchema,workflowNodeSchema } from "@/modules/workflows/contracts";
import { getWorkflowDetail,updateWorkflow } from "@/modules/workflows/use-cases";
import { getAdapter } from "@/server/infrastructure/providers";

import { workflowErrorResponse } from "../../route-support";
import { encodeEvent,errorMessage,paramsSchema } from "./route.params-schema";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ workflowId: string }> }) {
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      const parsedBody = workflowAgenticRequestSchema.safeParse(await req.json());
      ...
      const forbidden = await requireResourcePermissionAsync(session.user.id, workspaceId, "workflows.update", "workflow", (await params).workflowId);
      if (forbidden) return forbidden;

      const [workflow, availableAgents, configuredBuilderAgentId, history, currentTodoList] = await Promise.all([
      ...
      let builderAgent = configuredBuilderAgentId ? await getAgentById(configuredBuilderAgentId, workspaceId) : null;
      let version = builderAgent ? await getActiveVersion(builderAgent.id) : null;
      let provider = version ? await resolveProviderForVersion(version) : null;

      if (configuredBuilderAgentId && !builderAgent) {
        return NextResponse.json(
          {
            error: "The workflow builder assistant configured by an administrator is unavailable",
          },
          { status: 400 },
        );
      }
      ...
      if (configuredBuilderAgentId && (!version || !provider?.modelId)) {
        return NextResponse.json(
          {
            error: "The workflow builder assistant configured by an administrator requires an active model",
          },
          { status: 400 },
        );
      }
      ...
      if (!builderAgent) {
        const preferences = await getAgentDefaultPreferences(workspaceId, session.user.id, availableAgentIds);
        const preferredAgent = availableAgents.find((agent) => agent.id === preferences.effectiveDefaultAgentId);
        const candidates = [...(preferredAgent ? [preferredAgent] : []), ...availableAgents.filter((agent) => agent.id !== preferredAgent?.id)];

        for (const candidate of candidates) {
          const candidateVersion = await getActiveVersion(candidate.id);
          if (!candidateVersion) continue;
          const candidateProvider = await resolveProviderForVersion(candidateVersion);
      ...
      if (!builderAgent || !version || !provider?.modelId) {
        return NextResponse.json({ error: "No ready assistant is available for agentic mode" }, { status: 400 });
      }
      ...
      let initialWebResearch: Awaited<ReturnType<typeof searchWebWithSearxng>> | null = null;
      ...
      try {
        ...
        if (!initialWebResearch.ok) {
          initialWebResearchError = initialWebResearch.error ?? "No web search results were returned.";
        }
      } catch (error) {
        initialWebResearchError = error instanceof Error ? error.message : String(error);
      ...
      const system = [
        ...
        `Available assistants: ${JSON.stringify(availableAgents.map((agent) => ({ id: agent.id, name: agent.name })))}`,
        ...
        initialWebResearch?.ok ? `Fresh web research for this turn: ${JSON.stringify(initialWebResearch).slice(0, 16_000)}` : `The automatic web search attempt failed: ${initialWebResearchError ?? "unknown error"}. Call web_search before making claims that need external information.`,
      ].join("\n\n");
      ...
      const result = streamText({
        ...
        tools: {
          web_search: tool({
            description: "Search the live web for current, external, or implementation information. Use this whenever the automatic research is insufficient and cite useful result URLs in the final Markdown response.",
          ...
          set_workflow_plan: tool({
            description: "Record the required implementation and verification plan before editing the workflow.",
          ...
          update_todo_list: tool({
            description: "Create or replace the visible to-do list for this workflow task. Keep item IDs stable and update statuses after each milestone so the user sees live progress.",
          ...
          run_code_sandbox: tool({
            description: "Run a small Python, Node.js, or Bash test in the isolated sandbox. Use synthetic data only; never include secrets, private URLs, opaque secret references, or customer data.",
          ...
          request_user_input: tool({
            description: "Request essential structured information from the user. Sensitive fields open masked inputs and return only opaque references; ordinary fields can be returned in clear text.",
            inputSchema: z.object({
          ...
          update_workflow_details: tool({
            description: "Update the current workflow name or description without changing its graph.",
            inputSchema: z
              .object({
                name: z.string().trim().min(1).max(255).optional(),
                description: z.string().trim().max(2_000).nullable().optional(),
              })
              .refine((value) => value.name !== undefined || value.description !== undefined, "Provide a name or description."),
            execute: async (input) => {
              markDraftChanged();
              reserveAction();
              draft = {
          ...
          replace_workflow: tool({
            description: "Replace the complete workflow graph with a valid new definition.",
            inputSchema: z.object({
          ...
          upsert_workflow_nodes: tool({
            description: "Add new workflow steps or replace existing steps by id while preserving all other steps and connections.",
            inputSchema: z.object({
              summary: z.string().trim().min(1).max(240),
            ...
            execute: async ({ nodes }) => {
              markDraftChanged();
              reserveAction();
              const byId = new Map(draft.definition.nodes.map((node) => [node.id, node]));
          ...
          remove_workflow_nodes: tool({
            description: "Remove workflow steps by id and remove their attached connections. The manual trigger cannot be removed.",
            inputSchema: z.object({
            ...
            execute: async ({ nodeIds }) => {
              ...
              if (draft.definition.nodes.some((node) => removedIds.has(node.id) && node.type === "trigger.manual")) {
              ...
              draft = {
                ...
                definition: workflowDefinitionSchema.parse({
                  ...draft.definition,
                  nodes: draft.definition.nodes.filter((node) => !removedIds.has(node.id)),
                  edges: draft.definition.edges.filter((edge) => !removedIds.has(edge.source) && !removedIds.has(edge.target)),
                }),
              };
              revision += 1;
          ...
          connect_workflow_nodes: tool({
            description: "Replace all workflow connections using only source and target step IDs. For a condition step, set outcome to true or false; omit outcome for every other step. Edge IDs are generated automatically. Include every connection needed to make all useful steps reachable from the manual trigger.",
            inputSchema: z.object({
              connections: z
                .array(
                  z.object({
            ...
            execute: async ({ connections }) => {
              ...
              const nextDefinition = validateDefinition({
                ...
                edges: connections.map((connection, index) => ({
                  ...
                  sourceHandle: connection.outcome === "" || connection.outcome === undefined ? null : connection.outcome,
                })),
              });
              draft = {
                ...draft,
          ...
          replace_workflow_edges: tool({
            description: "Low-level fallback that replaces all workflow connections with explicit edge IDs. Prefer connect_workflow_nodes.",
            inputSchema: z.object({
          ...
          request_workflow_run: tool({
            ...
            execute: async ({ title, reason, input }) => {
              ...
              if (!workflowValidated || !dryRunCompleted) {
                throw new Error("Validate and dry-run the workflow before requesting execution.");
              }
              if (runRequestCreated) {
                throw new Error("Only one workflow execution request is allowed per turn.");
              }
              ...
              pendingRunRequest = {
                ...
                reason,
                payload: input,
                expectedVersion: workflow.latestVersion + (revision > 0 ? 1 : 0),
              };
              return {
                ok: true,
      ...
      const stream = new ReadableStream<Uint8Array>({
        async start(controller) {
          ...
          try {
            controller.enqueue(encodeEvent({ type: "agent", name: builderAgent.name }));
            controller.enqueue(
              encodeEvent({
                type: "tool_start",
              ...
              if (part.type === "text-delta") {
                ...
                controller.enqueue(
                  encodeEvent({
                    ...
                    label: workflowAgentToolLabels[part.toolName] ?? part.toolName,
                  }),
                );
                ...
                if (part.toolName === "update_todo_list" && typeof part.output === "object" && part.output !== null) {
                  controller.enqueue(
                    encodeEvent({
                      type: "todo_list",
            ...
            if (!assistantText.trim()) {
              const fallback = requestedInput ? "J’ai besoin des informations demandées pour continuer." : requestedRun ? "Le workflow est testé. J’attends votre validation avant de lancer l’exécution." : revision > 0 ? "Le workflow a été mis à jour." : "La demande a été analysée.";
