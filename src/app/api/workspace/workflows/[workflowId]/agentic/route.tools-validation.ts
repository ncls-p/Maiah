...
export function createWorkflowValidationTools(
  ...
  return {
    validate_workflow: tool({
      ...
      execute: async () => {
        ...
        state.draft = {
          ...state.draft,
          definition: state.validateDefinition(state.draft.definition),
        };
    ...
    dry_run_workflow: tool({
      ...
      execute: async ({ testInput }) => {
        ...
        return {
          ...
          revision: state.revision,
          testInput,
          nodes: state.draft.definition.nodes.map(({ id, type, label }) => ({
            id,
            type,
            label,
          })),
          connections: state.draft.definition.edges.map(
    ...
    request_workflow_run: tool({
      ...
      execute: async ({ title, reason, input }) => {
        state.requirePlan();
        if (!state.workflowValidated || !state.dryRunCompleted)
          throw new Error(
            "Validate and dry-run the workflow before requesting execution.",
          );
        if (state.runRequestCreated)
          throw new Error(
        ...
        state.pendingRunRequest = {
          ...
          reason,
          payload: input,
          expectedVersion: latestVersion + (state.revision > 0 ? 1 : 0),
        };
        return {
          ok: true,
          approvalRequired: true,
          expectedVersion: state.pendingRunRequest.expectedVersion,
...
export function createWorkflowAgentTools(context: {
  state: WorkflowAgenticState;
  workflowId: string;
  workspaceId: string;
  userId: string;
  ...
  return {
    ...createWorkflowBaseTools(context),
    ...createWorkflowEditTools(context.state),
    ...createWorkflowValidationTools(context.state, context.latestVersion),
  };
}
