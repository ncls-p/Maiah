...
export function createWorkflowAgentStream<
...
>(input: {
  ...
  return new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        controller.enqueue(
          encodeEvent({ type: "agent", name: input.builderAgentName }),
        );
        controller.enqueue(
          encodeEvent({
            type: "tool_start",
        ...
        controller.enqueue(
          encodeEvent({
            ...
            toolName: "web_search",
            label: workflowAgentToolLabels.web_search,
            status: input.initialWebResearchError ? "error" : "done",
          }),
        );
        for await (const part of result.stream) {
          if (part.type === "text-delta") {
            ...
            if (state.revision > 0 && state.revision !== streamedRevision) {
              ...
              controller.enqueue(
                encodeEvent({ type: "workflow", draft: state.draft }),
              );
            }
        ...
        if (state.revision > 0) {
          state.draft = {
            ...state.draft,
            name: z.string().trim().min(1).max(255).parse(state.draft.name),
