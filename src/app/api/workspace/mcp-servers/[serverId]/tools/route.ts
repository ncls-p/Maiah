...
export async function GET(req: NextRequest, { params }: { params: Promise<{ serverId: string }> }) {
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      return NextResponse.json(await listMcpTools(serverId, parsed.data.workspaceId, session.user.id));
    },
    {
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ serverId: string }> }) {
  return handleRoute(
    ...
    async ({ session }) => {
      const parsed = querySchema.safeParse({
        workspaceId: req.nextUrl.searchParams.get("workspaceId") ?? (await req.json().catch(() => ({}))).workspaceId,
      });
      if (!parsed.success) return NextResponse.json({ error: "workspaceId must be a valid UUID" }, { status: 400 });
      ...
      return NextResponse.json(await syncMcpTools(serverId, parsed.data.workspaceId, session.user.id, canManageGlobal));
    },
    {
