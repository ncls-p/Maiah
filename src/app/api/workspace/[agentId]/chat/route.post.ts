...
export async function POST(req: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  const jsonResponse = (body: unknown, status: number) => NextResponse.json(body, { status, headers: { "x-request-id": requestId } });
  const rejectChatRequest = (status: number, reason: string, body: unknown, context: Record<string, unknown> = {}) => {
    logger.warn("Chat request rejected", { requestId, status, reason, durationMs: Date.now() - requestStartedAt, ...context });
  ...
  try {
    ...
    if (forbidden) {
      logger.warn("Chat request rejected", { requestId, status: forbidden.status, reason: "missing_workspace_permission", agentId, workspaceId: agent.workspaceId, userId: actorUserId, durationMs: Date.now() - requestStartedAt });
    ...
    if (!quota.allowed) {
      return rejectChatRequest(429, "quota_exceeded", { error: quota.message, code: "quota_exceeded", used: quota.used, limit: quota.limit }, { agentId, workspaceId: agent.workspaceId, userId: actorUserId });
    }
    ...
    if (codeWorkspaceId) {
      ...
      if (metadata.workspaceId !== agent.workspaceId || metadata.createdByUserId !== actorUserId) {
        return rejectChatRequest(404, "code_workspace_not_found", { error: "Code workspace not found" }, { agentId, workspaceId: agent.workspaceId, userId: actorUserId, codeWorkspaceId });
      }
    ...
    for (const attachmentId of requestedAttachmentIds) {
      ...
      if (metadata.workspaceId !== agent.workspaceId || metadata.createdByUserId !== actorUserId) {
        return rejectChatRequest(404, "attachment_not_found", { error: "Attachment not found" }, { agentId, workspaceId: agent.workspaceId, userId: actorUserId, attachmentId });
      }
      messageAttachments.push(publicChatAttachment(metadata));
    }

    const preparedConversation = await prepareChatConversation({ agent, actorUserId, agentId, content, existingConversationId, ephemeral, ephemeralTtlMinutes, resendFromMessageId, continueFromMessageId, codeWorkspaceAttachment, messageAttachments, rejectChatRequest });
    ...
    const model = wrapLanguageModel({ model: adapter.createChatModel(providerConfig.runtimeConfig, providerConfig.modelId), middleware: extractReasoningMiddleware({ tagName: "think" }) });
    const memoryPolicy = version.memoryPolicyJson as { enabled?: boolean; summaryThresholdTokens?: number } | null;
    ...
    const availableAttachments = agent.kind === "orchestrator" ? await loadAuthorizedOrchestratorAttachments({ conversationId: conversation.id, workspaceId: agent.workspaceId, userId: actorUserId, current: messageAttachments }) : messageAttachments;

    const enqueueEvent = (event: Record<string, unknown>) => publishChatStreamEvent(assistantMessage.id, event);

    const executionContext = { requestId, agentId, actorUserId, agent, version, providerConfig, conversation, userMessage, assistantMessage, continuationClaim, content, history, generationHistory, availableAttachments, useAiSdkUIStream, shouldRegenerateConversationTitle, capabilityOverrides };
    ...
    logHandledError("Chat request failed", { requestId, status: 500, userMessageId, assistantMessageId, durationMs: Date.now() - requestStartedAt }, error as Error);

    return jsonResponse({ error: "Internal server error", ...(process.env.NODE_ENV !== "production" && error instanceof Error ? { detail: error.message } : {}) }, 500);
  }
}
