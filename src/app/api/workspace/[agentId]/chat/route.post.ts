...
import { AgentExecutionError,executeAgent,type AgentToolProgressEvent } from "@/modules/agent/runtime-executor";
...
import { canUseAgent,getActiveVersion,recordUsageEvent,resolveProviderForVersion } from "@/modules/agent/use-cases";
...
import { getChatAttachment,isChatFileAttachment,publicChatAttachment,type ChatAttachment } from "@/modules/chat/attachments";
import { generateChatAutomationArtifacts } from "@/modules/chat/automation";
import { claimAssistantContinuation,type AssistantContinuationClaim } from "@/modules/chat/continuation";
import { completeChatStream,createChatStreamResponse,createChatUIMessageStreamResponse,publishChatStreamEvent,registerChatStreamAbortController } from "@/modules/chat/stream-bus";
...
import { projectToolMessagePayload,safeChatErrorMessage,safeToolErrorMessage } from "@/modules/tool/safe-payload";
import { assertWorkspaceWithinTokenQuota } from "@/modules/usage/quota";
import { db } from "@/server/infrastructure/db";
import { agents,aiModels,conversations,messageParts,messages,toolInvocations,usageEvents } from "@/server/infrastructure/db/schema";
import { getAdapter } from "@/server/infrastructure/providers";
import { extractReasoningMiddleware,stepCountIs,ToolLoopAgent,wrapLanguageModel,type ToolSet } from "ai";
import { and,eq,gt,inArray,ne } from "drizzle-orm";
import { cookies } from "next/headers";
import { after,NextRequest,NextResponse } from "next/server";
import { loadConversationHistory } from "./route-history";
import { buildBoundTools,chatRequestSchema,codeWorkspaceCreateToolNames,defaultMaxOutputTokens,defaultMaxToolCalls,findUserMessageForResend,isFirstUserMessageInConversation,KNOWLEDGE_CONTEXT_TOOL_NAME,KNOWLEDGE_SEARCH_TOOL_NAME,knowledgeCitationsFromToolOutput,mergeUserFilePartMetadata,projectStreamedToolInput,streamToolCallId,streamToolErrorOutput,streamToolInputDelta } from "./route-support";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  try {
    ...
    if (agent.kind === "orchestrator") {
      ...
      const initialOrchestrationSortOrder = continuationClaim?.nextSortOrder ?? 0;
      ...
      const projectOrchestrationProgress = (progress: AgentToolProgressEvent) => {
        const isStart = progress.type === "tool-start";
        const status = isStart ? "running" : "error" in progress ? "error" : "success";
        const value = isStart
          ? progress.input
          : "error" in progress
            ? {
        ...
        const modelHistoryMetadata = progress.modelHistoryKind ? { modelHistoryKind: progress.modelHistoryKind } : {};
      ...
      const persistOrchestrationProgress = async (progress: AgentToolProgressEvent, sortOrder: number, citationSortOrder?: number) => {
        ...
        try {
          if (progress.modelHistoryKind !== "delegation-result") {
            await db.insert(messageParts).values({
              ...
              contentEncrypted: await encryptValue(JSON.stringify(projected.rawMetadata)),
              metadataJson: projected.safeMetadata,
              sortOrder,
            });
          }
          const knowledgeCitations = progress.type === "tool-end" && "output" in progress ? knowledgeCitationsFromToolOutput(progress.output) : [];
          if (knowledgeCitations.length > 0 && citationSortOrder !== undefined) {
            await db.insert(messageParts).values({
              messageId: assistantMessage.id,
              type: "citations",
              contentEncrypted: await encryptValue(JSON.stringify(knowledgeCitations)),
              metadataJson: null,
      ...
      const queueOrchestrationProgress = (progress: AgentToolProgressEvent) => {
        ...
        const citationSortOrder = progress.type === "tool-end" && "output" in progress && knowledgeCitationsFromToolOutput(progress.output).length > 0 ? allocateOrchestrationSortOrder() : undefined;
        if (progress.modelHistoryKind === "delegation-result") {
          durableDelegationProgress.push({ progress, sortOrder });
        }
        orchestrationProgressQueue = orchestrationProgressQueue
          .then(() => persistOrchestrationProgress(progress, sortOrder, citationSortOrder))
          .catch((error) => {
        ...
        try {
          ...
          const durableDelegationParts = await Promise.all(
            durableDelegationProgress.map(async ({ progress, sortOrder }) => {
              ...
              return {
                messageId: assistantMessage.id,
                type: projected.isStart ? ("tool-call" as const) : ("tool-result" as const),
                contentEncrypted: await encryptValue(JSON.stringify(projected.rawMetadata)),
                metadataJson: projected.safeMetadata,
                sortOrder,
              };
          ...
          await db.transaction(async (tx) => {
            ...
            if (result.text && continuationClaim?.appendableTextPart && nextOrchestrationSortOrder === initialOrchestrationSortOrder) {
              await tx
                .update(messageParts)
                .set({
                  contentEncrypted: await encryptValue(`${continuationClaim.appendableTextPart.content}${result.text}`),
                })
            ...
            await tx
              ...
              .set({
                status: "completed",
                tokenInput: accumulateTokenCount(continuationClaim?.message.tokenInput ?? null, result.inputTokens),
                tokenOutput: accumulateTokenCount(continuationClaim?.message.tokenOutput ?? null, result.outputTokens),
                completedAt,
              })
          ...
          if (aborted) {
            ...
            enqueueEvent({
              type: "error",
              error: completedRun ? "The agent run completed, but its response could not be saved. Open the run history to recover the result." : safeToolErrorMessage(error, "Orchestration failed. Review the run trace and try again."),
            });
          }
          logHandledError(
            "Orchestrator chat run failed",
            {
              ...
              conversationId: conversation.id,
              assistantMessageId: assistantMessage.id,
              errorCode: error instanceof AgentExecutionError ? error.code : "AGENT_RUN_FAILED",
              runId: error instanceof AgentExecutionError ? (error.runId ?? null) : null,
              errorDetail: error instanceof AgentExecutionError ? (error.safeDetail ?? null) : safeToolErrorMessage(error, "Agent run failed"),
            },
            error as Error,
          );
        } finally {
    ...
    const boundToolConfig = shouldUseToolCalling
      ? await buildBoundTools({
          ...
          disabledToolKeys,
          disabledSkillIds,
          enableDocumentExplorer: messageAttachments.some((attachment) => isChatFileAttachment(attachment) && attachment.extractedTextChars > 0) || history.some((message) => JSON.stringify(message).includes("Embedding-free document explorer:")),
          approvalPolicy,
    ...
    logger.info("Chat request accepted", {
      ...
      knowledgeToolsEnabled: availableToolNames.includes(KNOWLEDGE_SEARCH_TOOL_NAME),
      toolCount: availableToolNames.length,
      maxToolCalls,
      durationMs: Date.now() - requestStartedAt,
    });
    const versionToolChoice = version.toolChoice;
    const configuredToolChoice: "auto" | "required" | "none" | undefined = availableToolNames.length > 0 ? (versionToolChoice === "required" || versionToolChoice === "none" ? versionToolChoice : "auto") : undefined;
    const businessArtifactToolNames = ["create_business_document", "create_spreadsheet", "create_meeting_brief", "create_action_plan", "create_decision_matrix", "create_email_pack", "create_project_status_report", "create_risk_register", "create_raci_matrix", "create_customer_account_plan", "create_competitive_battlecard"];
    const codeWorkspaceToolNames = codeWorkspaceCreateToolNames;
    const hasBusinessArtifactTools = businessArtifactToolNames.some((name) => availableToolNames.includes(name));
    const hasCodeWorkspaceTools = codeWorkspaceToolNames.some((name) => availableToolNames.includes(name));
    const toolGuidance =
      availableToolNames.length > 0
        ? [
            ...
            availableToolNames.includes("web_search") ? "For web or current-events searches, use web_search only." : null,
            availableToolNames.includes(KNOWLEDGE_SEARCH_TOOL_NAME) ? `Connected data sources are available through ${KNOWLEDGE_SEARCH_TOOL_NAME}. Call it only when the request may depend on those sources; do not query data sources automatically for every message. Choose one or more knowledgeBaseIds explicitly from the source names and descriptions exposed by the tool, using multiple sources only when useful. Search results include chunk IDs. When surrounding context is necessary, call ${KNOWLEDGE_CONTEXT_TOOL_NAME} with one of those IDs and a bounded number of chunks before or after.` : null,
            availableToolNames.includes("update_todo_list") ? "For tasks with multiple meaningful steps, call update_todo_list early, keep the same item IDs, mark exactly one current item in_progress when possible, and call it again as items are completed. Do not create a checklist for a simple one-step answer." : null,
            availableToolNames.includes("create_slide_deck") ? "When the user asks for slides, a deck, presentation, pitch deck, PDF slides, or follow-up edits to an existing deck, use create_slide_deck. It creates an interactive click-through HTML deck with print-to-PDF styling; explain briefly that PDF export is static because modern PDF viewers do not preserve JavaScript click animations." : null,
            hasBusinessArtifactTools ? "For common business deliverables, prefer the dedicated artifact tools instead of plain prose: create_business_document for briefs/reports/proposals/policies/SOPs, create_spreadsheet for structured tables, create_meeting_brief for agendas/minutes/action items, create_action_plan for phased execution plans, create_decision_matrix for option comparisons, create_email_pack for professional email drafts, create_project_status_report for steering updates, create_risk_register for risk tracking, create_raci_matrix for role clarity, create_customer_account_plan for sales/account strategy, and create_competitive_battlecard for competitive sales enablement." : null,
            availableToolNames.includes("render_html_artifact") ? "When the user asks for a visual design, diagram, UI mockup, chart-like schema, or interactive demo that is not specifically a slide deck, use render_html_artifact with self-contained HTML, CSS, and optional JavaScript so it appears directly in the chat. The user can view and copy the code from the artifact card, so do not duplicate the full code in your final text unless explicitly asked." : null,
    ...
    const responseFormatGuidance = responseFormat?.type === "json_object" ? "Respond with a valid JSON object only. Do not include markdown fences or explanatory prose outside the JSON object." : null;
    const guardrailGuidance = guardrails?.enabled && guardrails.blockedTopics?.length ? `Avoid and refuse requests about these blocked topics: ${guardrails.blockedTopics.join(", ")}.` : null;
    const localeCookie = (await cookies()).get("NEXT_LOCALE")?.value ?? "en";
    const systemPrompt = [version.systemPrompt?.trim() || fallbackSystemPrompt(localeCookie), skillsPrompt, responseFormatGuidance, guardrailGuidance, toolGuidance].filter(Boolean).join("\n\n");
    ...
    let appendableContinuationTextPart = continuationClaim?.appendableTextPart ?? null;

    async function appendStreamedTextPart(type: "text" | "reasoning", content: string) {
      if (type === "text" && streamedParts.length === 0 && appendableContinuationTextPart) {
        appendableContinuationTextPart.content += content;
        await db
          .update(messageParts)
          .set({
            contentEncrypted: await encryptValue(appendableContinuationTextPart.content),
          })
    ...
    async function appendStreamedMetadataPart(type: "tool-call" | "tool-result" | "file", metadata: unknown) {
      appendableContinuationTextPart = null;
      const safeMetadata = type === "file" ? metadata : projectToolMessagePayload(metadata);
      const [inserted] = await db
        .insert(messageParts)
        .values({
          messageId: assistantMessage.id,
          type,
          contentEncrypted: type === "file" ? null : await encryptValue(JSON.stringify(metadata ?? null)),
    ...
    void (async () => {
      try {
        ...
        const displayedUsageImpact = calculateTokenUsageImpact({
          inputTokens: (continuationClaim?.message.tokenInput ?? 0) + (totalUsage.inputTokens ?? 0),
          outputTokens: (continuationClaim?.message.tokenOutput ?? 0) + (totalUsage.outputTokens ?? 0),
          inputCostPerMillion: usageModel?.inputTokenCost,
          outputCostPerMillion: usageModel?.outputTokenCost,
          sustainability,
          currency: sustainability.currency,
        ...
        const assistantText = streamedParts
          .flatMap((part) => (part.type === "text" && "content" in part ? [part.content] : []))
          .join("\n")
          .trim();
        postCompletionAutomationRef.current = async () => {
          ...
          if (shouldRegenerateConversationTitle && generatedTitle.trim() && generatedTitle.trim() !== conversation.title.trim()) {
            await db.update(conversations).set({ title: generatedTitle, updatedAt: new Date() }).where(eq(conversations.id, conversation.id));
          }
        };
        ...
        await db.transaction(async (tx) => {
          await tx
            ...
            .set({
              status: "completed",
              tokenInput: accumulateTokenCount(continuationClaim?.message.tokenInput ?? null, totalUsage.inputTokens),
              tokenOutput: accumulateTokenCount(continuationClaim?.message.tokenOutput ?? null, totalUsage.outputTokens),
              completedAt,
            })
            .where(eq(messages.id, assistantMessage.id));
          ...
          await tx.insert(usageEvents).values({
            ...
            costUsd: eventUsageImpact.cost === null || eventUsageImpact.currency !== "USD" ? null : String(eventUsageImpact.cost),
          ...
          if (usageImpactSetting.enabled) {
            await tx.insert(messageParts).values({
              messageId: assistantMessage.id,
              type: "impact",
              contentEncrypted: await encryptValue(JSON.stringify(displayedUsageImpact)),
