...
export type ChatExecutionContext = { requestId: string; agentId: string; actorUserId: string; agent: ChatAgentRow; version: ChatAgentVersion; providerConfig: ChatProviderConfig; conversation: ChatConversationRow; userMessage: ChatMessageRow | null; assistantMessage: ChatMessageRow; continuationClaim: ClaimedContinuation | null; content: string; history: ModelMessage[]; generationHistory: ModelMessage[]; availableAttachments: ChatAttachment[]; useAiSdkUIStream: boolean; shouldRegenerateConversationTitle: boolean; capabilityOverrides?: { disabledTools: Array<{ source: "builtin" | "mcp" | "custom"; id: string }>; disabledSkillIds: string[]; enabledTools: Array<{ source: "builtin" | "mcp" | "custom"; id: string }>; enabledSkillIds: string[]; enabledKnowledgeIds: string[] } };

export function chatStreamHeaders(context: ChatExecutionContext) {
  return { "X-Conversation-Id": context.conversation.id, "X-Message-Id": context.assistantMessage.id, ...(context.userMessage ? { "X-User-Message-Id": context.userMessage.id } : {}), "X-Request-Id": context.requestId };
}
