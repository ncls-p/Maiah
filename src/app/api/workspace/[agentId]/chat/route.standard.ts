...
export async function runStandardChat(input: { context: ChatExecutionContext; model: LanguageModel; messageAttachments: ChatAttachment[]; createdConversation: boolean; codeWorkspaceAttachment: unknown; requestStartedAt: number; enqueueEvent: (event: Record<string, unknown>) => void }) {
  ...
  const { maxToolCalls, maxOutputTokens, maxSteps, boundToolConfig, tools, availableToolNames, configuredToolChoice, systemPrompt } = await prepareStandardChatConfig({ context: executionContext, messageAttachments, createdConversation, hasCodeWorkspaceAttachment: Boolean(codeWorkspaceAttachment), requestStartedAt, enqueueEvent });
...
void (async () => {
  try {
    ...
    await completeStandardChat({ context: executionContext, totalUsage, partWriter, postCompletionAutomationRef, startedAt, enqueueEvent });
  } catch (error) {
