...
import { getProviderById,refreshProviderModels } from "@/modules/provider/use-cases";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ providerId: string }> }) {
  return handleRoute(
    ...
    async ({ session }) => {
      ...
      return NextResponse.json(await refreshProviderModels(providerId, workspaceId));
    },
