...
export type AgentProgressModelHistoryProjection = { kind: "visual-only" } | { kind: "delegation-result"; text: string } | null;
 ...
 */
export function projectAgentProgressForModelHistory(metadata: unknown): AgentProgressModelHistoryProjection {
  ...
  if ("modelHistoryKind" in metadata) {
    ...
    return text === null ? { kind: "visual-only" } : { kind: "delegation-result", text };
  }
