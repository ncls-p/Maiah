...
export async function updateAgentUnlocked(input: UpdateAgentInput) {
  ...
  if (existing.kind === "assistant" && (orchestrationPolicy !== undefined || (delegationBindings?.length ?? 0) > 0)) {
  ...
  const normalizedToolBindings = canAdminCurate
    ? toolBindings
    : await preserveBuiltinApprovalOverrides(toolBindings, existing.activeVersionId, {
        workspaceId,
        userId,
      });
  ...
  const { agent, version } = await db.transaction(async (tx) => {
    ...
    const [lockedAgent] = await tx
      ...
      .where(and(eq(agents.id, agentId), eq(agents.workspaceId, workspaceId), activeVersionPredicate))
      .returning();
    if (!lockedAgent) {
    ...
    if (promptSuggestions !== undefined) {
      agentUpdates.promptSuggestionsJson = preparePromptSuggestions(promptSuggestions);
    }
    if (sharingMode !== undefined) {
      agentUpdates.sharingMode = sharingMode;
      agentUpdates.shareTargetUserId = nextShareTargetUserId;
      agentUpdates.visibility = sharingMode === "marketplace" ? "public" : "private";
    }
    if (canAdminCurate) {
      ...
      if (curationLabel !== undefined || isRecommended !== undefined) {
        agentUpdates.curationLabel = normalizeCurationLabel(curationLabel, isRecommended ?? existing.isRecommended);
      }
    }
    ...
    const nextProviderId = providerId !== undefined ? providerId : (activeConfig?.providerId ?? null);
    const nextModelId = modelId !== undefined ? modelId : providerId !== undefined ? null : (activeConfig?.modelId ?? null);
    const nextOrchestrationPolicy = existing.kind === "orchestrator" ? normalizeOrchestrationPolicy(orchestrationPolicy ?? activeConfig?.orchestrationPolicyJson) : null;

    if (nextProviderId) {
      const [provider] = await tx
        .select({ id: aiProviders.id })
        .from(aiProviders)
        .where(and(eq(aiProviders.id, nextProviderId), eq(aiProviders.workspaceId, workspaceId), isNull(aiProviders.archivedAt)))
        .limit(1);
    ...
    if (nextModelId) {
      ...
      const [model] = await tx
        ...
        .from(aiModels)
        .where(and(eq(aiModels.id, nextModelId), eq(aiModels.providerId, nextProviderId), eq(aiModels.enabled, true)))
        .limit(1);
    ...
    const [version] = await tx
      ...
      .values({
        ...
        systemPrompt: systemPrompt !== undefined ? systemPrompt : (activeConfig?.systemPrompt ?? null),
        providerId: nextProviderId,
        modelId: nextModelId,
        temperature: temperature !== undefined ? temperature : (activeConfig?.temperature ?? null),
        topP: topP !== undefined ? topP : (activeConfig?.topP ?? null),
        maxOutputTokens: maxOutputTokens !== undefined ? maxOutputTokens : (activeConfig?.maxOutputTokens ?? null),
        maxToolCalls: maxToolCalls !== undefined ? maxToolCalls : (activeConfig?.maxToolCalls ?? 20),
        toolChoice: toolChoice !== undefined ? toolChoice : (activeConfig?.toolChoice ?? null),
        generationSettingsJson: generationSettings !== undefined ? generationSettings : (activeConfig?.generationSettingsJson ?? null),
        responseFormatJson: responseFormat !== undefined ? { type: responseFormat } : (activeConfig?.responseFormatJson ?? null),
        memoryPolicyJson: memoryPolicy !== undefined ? memoryPolicy : (activeConfig?.memoryPolicyJson ?? null),
        guardrailsJson: guardrails !== undefined ? guardrails : (activeConfig?.guardrailsJson ?? null),
        approvalPolicyJson: approvalPolicy !== undefined ? approvalPolicy : (activeConfig?.approvalPolicyJson ?? null),
    ...
    if (knowledgeBindings !== undefined) {
      ...
      await cloneKnowledgeBindings(baseVersionId, version.id, workspaceId, { userId }, tx);
    }

    if (skillBindings !== undefined) {
      await replaceSkillBindingsForVersion(version.id, workspaceId, skillBindings, { userId }, tx);
    } else {
      await cloneSkillBindings(baseVersionId, version.id, workspaceId, { userId }, tx);
    }
