...
export const orchestrationPolicySchema = z.object({
  ...
  maxDelegations: z.number().int().min(1).max(orchestrationPolicyCaps.maxDelegations),
  maxParallel: z.number().int().min(1).max(orchestrationPolicyCaps.maxParallel),
  maxChildSteps: z.number().int().min(2).max(orchestrationPolicyCaps.maxChildSteps),
  maxTotalTokens: z.number().int().min(1_000).max(orchestrationPolicyCaps.maxTotalTokens),
  timeoutMs: z.number().int().min(5_000).max(orchestrationPolicyCaps.timeoutMs),
  resultMaxChars: z.number().int().min(1_000).max(orchestrationPolicyCaps.resultMaxChars),
});
...
export function normalizeOrchestrationPolicy(value: unknown): OrchestrationPolicy {
  ...
  return orchestrationPolicySchema.parse({
    ...
    ...(typeof legacyChildSteps === "number" ? { maxChildSteps: Math.max(2, legacyChildSteps) } : {}),
  });
}
...
export type DelegationBindingInput = z.infer<typeof delegationBindingInputSchema>;
