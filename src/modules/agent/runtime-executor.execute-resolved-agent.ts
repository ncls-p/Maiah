...
export async function executeResolvedAgent(input: InternalExecutionInput): Promise<AgentExecutionResult> {
  ...
  try {
    ...
    const runtimeLimits = resolveAgentRuntimeLimits({ maxToolCalls: input.resolved.version.maxToolCalls, maxOutputTokens: input.resolved.version.maxOutputTokens });
    ...
    const bound = !input.dryRun && runtimeLimits.maxToolCalls > 0 ? await buildBoundTools({ agentVersionId: input.resolved.version.id, workspaceId: input.workspaceId, conversationId: input.conversationId ?? undefined, messageId: input.messageId ?? undefined, userId: input.userId, maxToolCalls: runtimeLimits.maxToolCalls, approvalPolicy: (input.resolved.version.approvalPolicyJson as never) ?? null, hasSkills: Boolean(skillsPrompt), enableDocumentExplorer: (input.availableAttachments?.length ?? 0) > 0, nonInteractive: true }) : { tools: {}, toolApproval: undefined };
    const delegationTools = await buildDelegationTools({ runId, resolved: input.resolved, execution: input, allocateSequence, onToolSuccess: recordSuccessfulToolResult });
      ...
      result = await generateText({
        ...
        prepareStep: hasTools
          ? ({ stepNumber }) => {
              ...
              return { activeTools: [], toolChoice: "none", instructions: `${system}\n\n${finalSynthesisInstruction}` };
            }
          : undefined,
        ...
        onToolExecutionStart: ({ toolCall }) => emitToolProgress(input.onProgress, { type: "tool-start", id: `${runId}:${toolCall.toolCallId}`, toolCallId: toolCall.toolCallId, toolName: toolCall.toolName, agentName: input.resolved.agent.name, agentId: input.resolved.agent.id, runId, parentRunId: input.parentRunId ?? null, depth: input.depth, ...progressModelHistoryMetadata({ depth: input.depth, isDelegation: toolCall.toolName.startsWith("delegate_specialist_"), phase: "start" }), input: toolCall.input }),
        onToolExecutionEnd: ({ toolCall, toolExecutionMs, toolOutput }) => {
          const context = { id: `${runId}:${toolCall.toolCallId}`, toolCallId: toolCall.toolCallId, toolName: toolCall.toolName, agentName: input.resolved.agent.name, agentId: input.resolved.agent.id, runId, parentRunId: input.parentRunId ?? null, depth: input.depth, ...progressModelHistoryMetadata({ depth: input.depth, isDelegation: toolCall.toolName.startsWith("delegate_specialist_"), phase: toolOutput.type === "tool-error" ? "error" : "success" }) } satisfies AgentToolProgressContext;
          if (toolOutput.type === "tool-error") {
            const executionError = toolOutput.error instanceof AgentExecutionError ? toolOutput.error : null;
            emitToolProgress(input.onProgress, { ...context, type: "tool-end", durationMs: toolExecutionMs, error: executionError?.safeDetail ? safeToolErrorMessage(new Error(executionError.safeDetail), "Tool execution failed") : safeToolErrorMessage(toolOutput.error, "Tool execution failed"), ...(executionError?.code ? { errorCode: executionError.code } : {}) });
            return;
          }
          emitToolProgress(input.onProgress, { ...context, type: "tool-end", durationMs: toolExecutionMs, output: toolOutput.output });
        },
        telemetry: { functionId: "ai-hub.agent-run", recordInputs: false, recordOutputs: false },
      });
    } catch (error) {
    ...
    if (result) {
      ...
      if (successfulToolResults.length === 0 && (result.toolResults?.length ?? 0) > 0) {
        successfulToolResults.push(...result.toolResults.map((toolResult) => ({ toolName: toolResult.toolName, output: toolResult.output })));
      }
      if (!text && successfulToolResults.length > 0) {
        ...
        if (recoveryRemainingTokens > 0 && !deadline.signal.aborted) {
          try {
            const recoveryResult = await generateText({ model, system: `${system}\n\n${emptyResponseRecoveryInstruction}`, prompt: ["Original task:", input.prompt, "Successful tool results:", toolResultRecoveryContext(successfulToolResults, input.budget.policy.resultMaxChars)].join("\n\n"), temperature: input.resolved.version.temperature ? Number.parseFloat(input.resolved.version.temperature) : undefined, topP: input.resolved.version.topP ? Number.parseFloat(input.resolved.version.topP) : undefined, maxOutputTokens: Math.max(1, Math.min(runtimeLimits.maxOutputTokens, recoveryRemainingTokens)), abortSignal: deadline.signal, telemetry: { functionId: "ai-hub.agent-run.empty-response-recovery", recordInputs: false, recordOutputs: false } });
    ...
    await appendAgentRunStep({ runId, sequence: allocateSequence(), kind: "model", status: "success", name: provider.modelId, inputPreview: { prompt: input.prompt }, outputPreview: { text, inputTokens, outputTokens, recoveredFromEmptyResponse, recoveredFromToolResult }, completedAt: new Date() });
    await completeAgentRun({ runId, output: { text }, inputTokens, outputTokens, reservationTokens: input.depth === 0 ? input.budget.tokensUsed : undefined, usage: { workspaceId: input.workspaceId, userId: input.userId, providerId: provider.providerId, modelId: provider.modelRecordId, agentId: input.resolved.agent.id, conversationId: input.conversationId ?? undefined, operation: input.trigger === "delegation" ? "delegation" : input.trigger, latencyMs: Date.now() - startedAt } });
    return { runId, text, inputTokens, outputTokens, totalTreeTokens: input.budget.tokensUsed, reused: false, visualOutputs: collectAgentVisualOutputs(successfulToolResults) };
  } catch (error) {
    const aborted = input.budget.controller.signal.aborted;
    await failAgentRun({ runId, status: aborted ? "cancelled" : "failed", error, errorCode: error instanceof AgentExecutionError ? error.code : aborted ? "AGENT_RUN_CANCELLED" : "AGENT_RUN_FAILED", inputTokens, outputTokens, reservationTokens: input.depth === 0 ? input.budget.tokensUsed : undefined, usage: { workspaceId: input.workspaceId, userId: input.userId, providerId: usageProvider?.providerId, modelId: usageProvider?.modelRecordId, agentId: input.resolved.agent.id, conversationId: input.conversationId ?? undefined, operation: input.trigger === "delegation" ? "delegation" : input.trigger, latencyMs: Date.now() - startedAt } });
