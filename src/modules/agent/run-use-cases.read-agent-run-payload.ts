...
import { agentRuns,workspaceTokenReservations } from "@/server/infrastructure/db/schema";
...
export async function readAgentRunPayload(runId: string) {
  ...
  return {
    ...
    output: run.outputEncrypted ? (JSON.parse(await decryptValue(run.outputEncrypted)) as unknown) : null,
  };
}
...
export async function reapExpiredAgentRuns(now = new Date()) {
  return db.transaction(async (tx) => {
    ...
    const expired = await tx
      ...
      .where(and(inArray(agentRuns.status, ["queued", "running", "waiting_approval"]), lt(agentRuns.deadlineAt, now)))
    ...
    const leaseLost = await tx
      ...
      .where(and(eq(agentRuns.status, "running"), lt(agentRuns.leaseExpiresAt, now)))
      .returning({ id: agentRuns.id });
    ...
    if (terminalRunIds.length > 0) {
      await tx
        .update(workspaceTokenReservations)
        .set({ status: "expired", updatedAt: now })
        .where(and(inArray(workspaceTokenReservations.runId, terminalRunIds), eq(workspaceTokenReservations.status, "active")));
    }
