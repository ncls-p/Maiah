...
import { roleBindings,teamMembers,teams } from "@/server/infrastructure/db/schema";
...
export async function removeTeamMember(input: { actorUserId: string; workspaceId: string; teamId: string; userId: string }) {
...
export async function deleteTeam(input: { actorUserId: string; workspaceId: string; teamId: string }) {
  const { organization } = await getWorkspaceScope(input.workspaceId);
  await requirePermission({
    userId: input.actorUserId,
    permission: "teams.manage",
  ...
  const affectedUsers = await db.select({ userId: teamMembers.userId }).from(teamMembers).where(eq(teamMembers.teamId, team.id));
  await db.transaction(async (tx) => {
    await tx.delete(roleBindings).where(and(eq(roleBindings.principalType, "group"), eq(roleBindings.principalId, team.id)));
    await tx.delete(teams).where(eq(teams.id, team.id));
  });

  await Promise.all(affectedUsers.map(({ userId }) => invalidateUserOrganizationAccess(userId, organization.id)));
  await audit.emit({
    organizationId: organization.id,
    workspaceId: input.workspaceId,
    actorPrincipalType: "user",
