...
export type TransferOwnershipPolicy = (typeof TRANSFER_OWNERSHIP_POLICIES)[number];
export type TransferSecretPolicy = (typeof TRANSFER_SECRET_POLICIES)[number];
export const RESOURCE_TRANSFER_ROOT_TYPES = [...ACCESS_RESOURCE_TYPES, "workspace"] as const;
export type ResourceTransferRootType = (typeof RESOURCE_TRANSFER_ROOT_TYPES)[number];
...
export type TransferSets = Record<AccessResourceType, Map<string, TransferSeed["reason"]>>;

export const RESOURCE_TYPES: AccessResourceType[] = ["agent", "provider", "model", "mcp_server", "tool_connector", "tool_connection", "custom_tool", "knowledge_base", "skill", "workflow", "scheduled_task", "conversation", "marketplace_item"];

export function emptyTransferSets(): TransferSets {
  return Object.fromEntries(RESOURCE_TYPES.map((type) => [type, new Map()])) as TransferSets;
}

export function addResource(sets: TransferSets, type: AccessResourceType, id: string | null | undefined, reason: TransferSeed["reason"]) {
...
export async function requireTransferPermission(userId: string, workspaceId: string) {
  ...
  if (!result.granted) {
    throw new IamOperationError("You need project access administration rights to transfer resources", 403);
  }
}

export async function listResourceTransferDestinations(input: { userId: string; sourceWorkspaceId: string }) {
  ...
  const manageable = await Promise.all(
    candidates.map(async ({ workspace, organization }) => ({
      workspace,
      organization,
      allowed: workspace.id !== input.sourceWorkspaceId && (await authorization.checkPermission({ principalType: "user", principalId: input.userId }, "roles.manage", "workspace", workspace.id)).granted,
    })),
  );
  ...
  return manageable
    ...
    .sort((a, b) => a.organizationName.localeCompare(b.organizationName) || a.workspaceName.localeCompare(b.workspaceName));
}
