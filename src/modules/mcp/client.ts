...
async function buildAuthHeaders(server: McpServerRow, extraHeaders: Record<string, string> = {}): Promise<Record<string, string>> {
...
function createTransport(url: URL, transport: McpTransport, headers: Record<string, string>): Transport {
  ...
  try {
    await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeoutId = setTimeout(() => reject(new Error("MCP connection timed out")), CONNECT_TIMEOUT_MS);
      }),
    ]);
  } finally {
...
async function connectClient(server: McpServerRow, options: McpClientOptions = {}): Promise<{ client: Client; transport: Transport }> {
  ...
  const fallbackTransport = server.transport === "streamable-http" ? createTransport(url, "sse", headers) : null;

  let lastError: unknown;
  for (const transport of [primaryTransport, fallbackTransport].filter(Boolean) as Transport[]) {
    try {
...
async function withMcpClient<T>(server: McpServerRow, fn: (client: Client) => Promise<T>, options: McpClientOptions = {}): Promise<T> {
...
export async function listRemoteMcpTools(server: McpServerRow): Promise<Tool[]> {
...
export async function callRemoteMcpTool(server: McpServerRow, toolName: string, toolInput: unknown, options: McpClientOptions = {}): Promise<CallToolResult> {
