...
import { agentKnowledgeBindings,agentSkillBindings,agentSkills,agentToolBindings,customTools,knowledgeBases,mcpServers,mcpTools } from "@/server/infrastructure/db/schema";
import { and,eq,inArray } from "drizzle-orm";
import { buildCustomToolManifest,buildMcpPresetManifest,buildSkillContentManifest,jsonRecord } from "./manifest-builders.json-record";
import { resolveAgentVersion,resolveToolBindingRef } from "./manifest-builders.resolve-agent-version";
import type { AgentMarketplaceManifest,McpPresetMarketplaceManifest,PortableKnowledgeBinding,PortableSkillBinding,PortableToolBinding,ToolMarketplaceManifest } from "./manifest-types";

export async function buildAgentManifest(agentId: string, workspaceId: string, name: string, description?: string | null): Promise<AgentMarketplaceManifest> {
  ...
  const toolBindings = await db.select().from(agentToolBindings).where(eq(agentToolBindings.agentVersionId, agentVersion.id));

  const skillBindingsRows = await db.select().from(agentSkillBindings).where(eq(agentSkillBindings.agentVersionId, agentVersion.id));

  const knowledgeBindingsRows = await db.select().from(agentKnowledgeBindings).where(eq(agentKnowledgeBindings.agentVersionId, agentVersion.id));
  ...
  const skills =
    skillIds.length > 0
      ? await db
          .select()
          .from(agentSkills)
          .where(and(inArray(agentSkills.id, skillIds), eq(agentSkills.workspaceId, workspaceId)))
      : [];
  ...
  const kbs =
    kbIds.length > 0
      ? await db
          .select()
          .from(knowledgeBases)
          .where(and(inArray(knowledgeBases.id, kbIds), eq(knowledgeBases.workspaceId, workspaceId)))
      : [];
  ...
  for (const binding of toolBindings) {
    if (binding.toolSource === "mcp") {
      const [tool] = await db.select().from(mcpTools).where(eq(mcpTools.id, binding.toolId)).limit(1);
      if (!tool || seenMcpServers.has(tool.mcpServerId)) continue;
      const [server] = await db
        .select()
        .from(mcpServers)
        .where(and(eq(mcpServers.id, tool.mcpServerId), eq(mcpServers.workspaceId, workspaceId)))
        .limit(1);
      if (!server) continue;
      seenMcpServers.add(server.id);
      const serverTools = await db.select().from(mcpTools).where(eq(mcpTools.mcpServerId, server.id));
      bundledMcpPresets.push(buildMcpPresetManifest(server.name, null, server, serverTools, "server"));
    }
    if (binding.toolSource === "custom") {
      ...
      bundledCustomTools.push(await buildCustomToolManifest(tool, tool.name, tool.description));
    }
  }
