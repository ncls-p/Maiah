...
import {
  ensureExternallyTriggerableWorkflow,
  extractWorkflowId,
  loadSecretPayloads,
  replaceSecretPlaceholders,
} from "./use-cases.compact-mcp-result";
import {
...
export function createCustomToolBuilderTools(context: {
  input: CustomToolBuilderInput;
  config: BuilderConfig;
  reserveBuilderAction: () => void;
  secretRequests: Array<{
    id: string;
    title: string;
  ...
  createdWorkflows: unknown[];
  workflowPreviews: WorkflowPreview[];
  registeredTools: Array<{ id: string; name: string; status: string }>;
  progressEvents: Array<{ label: string; status: "done" | "pending" }>;
}) {
  const {
    input,
    config,
