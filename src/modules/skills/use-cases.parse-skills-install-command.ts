...
import { GITHUB_PACKAGE_PATTERN,ParsedInstallCommand,SKILLS_CLI_FLAGS,SKILLS_CLI_VALUE_FLAGS,SkillPreviewResult,maxInstallCommandLength,normalizePackageAndSkill,tokenizeInstallCommand } from "./use-cases.exec-file-async";
...
function readSkillsCliPrefix(tokens: string[]) {
  ...
  if (tokens[index] === "skillsadd") {
    throw new Error("Use `npx skills add ...` with a space between skills and add");
  }
...
function normalizeSkillNames(skillNames: Set<string>) {
  const normalizedSkills = [...skillNames].map((name) => name.trim()).filter(Boolean);

  if (normalizedSkills.length === 0) {
    throw new Error("Choose a specific skill with `--skill <name>` or `owner/repo@skill`");
  }

  for (const skillName of normalizedSkills) {
    if (!SKILL_NAME_PATTERN.test(skillName) || skillName === "*") {
      throw new Error("Skill names must be explicit and contain only letters, numbers, dot, dash or underscore");
    }
  }
...
function normalizeSourcePackage(sourcePackage: string) {
  ...
  if (!GITHUB_PACKAGE_PATTERN.test(normalized)) {
    throw new Error("Only GitHub owner/repository skill packages are supported");
  }
  return normalized;
}

export function parseSkillsInstallCommand(command: string): ParsedInstallCommand {
...
export function checksumSkillPreview(skills: SkillPreviewResult[]) {
  const canonical = [...skills]
    ...
    .sort((a, b) => `${a.sourcePackage}/${a.name}`.localeCompare(`${b.sourcePackage}/${b.name}`));
  return sha256(JSON.stringify(canonical));
}

export function signSkillPreviewPayload(payload: string) {
  return createHmac("sha256", Buffer.from(env.APP_ENCRYPTION_KEY, "hex")).update(payload).digest("base64url");
}
