...
export async function loadBoundSkillContent(input: { agentVersionId: string; skillName: string; disabledSkillIds?: ReadonlySet<string>; enabledSkillIds?: ReadonlySet<string>; workspaceId?: string; userId?: string }) {
  ...
  const extraRows = extraIds.length > 0 && input.workspaceId && input.userId ? await db.select({ skill: agentSkills }).from(agentSkills).where(and(inArray(agentSkills.id, extraIds), eq(agentSkills.workspaceId, input.workspaceId), isNull(agentSkills.archivedAt))) : [];
