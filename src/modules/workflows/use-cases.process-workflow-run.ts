...
import { workflowRuns,workflowVersions } from "@/server/infrastructure/db/schema";

import { compileWorkflowDefinition,createWorkflowEventBus,createWorkflowRuntime,workflowNodeById } from "./runtime";
...
export async function processWorkflowRun(runId: string) {
  const [record] = await db.select({ run: workflowRuns, version: workflowVersions }).from(workflowRuns).innerJoin(workflowVersions, eq(workflowRuns.workflowVersionId, workflowVersions.id)).where(eq(workflowRuns.id, runId)).limit(1);
  ...
  try {
    ...
    await db.update(workflowRuns).set({ status: "running", startedAt: new Date(), error: null }).where(eq(workflowRuns.id, runId));
    ...
    const result = await runtime.run(blueprint, { input: record.run.inputJson ?? null }, { strict: true, concurrency: 4 });
    const completed = result.status === "completed";
    const failure = failureDetail ?? result.errors?.map((error) => error.message).join(" ");
    const [run] = await db
      .update(workflowRuns)
      .set({
...
export async function listQueuedWorkflowRunIds() {
  const rows = await db.select({ id: workflowRuns.id }).from(workflowRuns).where(eq(workflowRuns.status, "queued")).limit(500);
