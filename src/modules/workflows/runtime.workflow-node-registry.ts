...
import { calculateNumber,condition,currentDate,delayFlow,filterList,sliceList,sortList,stopFlow } from "./runtime.calculate-number";
import { debugSnapshot,executeCode,httpRequest,runAgent } from "./runtime.http-request";
import { manualTrigger,parseJson,pickData,removeData,renameData,setData,stringifyJson,templateData,transformText } from "./runtime.matches-comparison";
...
export function assertNodeParameters(node: WorkflowNode) {
  ...
  if (node.type === "agent.run") {
    if (typeof params.agentId !== "string" || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(params.agentId)) {
  ...
  if (node.type === "http.request") {
    ...
    if (params.headers !== undefined && Object.keys(objectValue(params.headers)).length > 50) {
  ...
  if (node.type === "code.execute") {
    ...
    if (typeof params.code !== "string" || !params.code.trim() || params.code.length > 100_000) {
      throw new Error(`Node '${node.label}' requires code under 100,000 characters.`);
    }
  }
  if (node.type === "data.set" && Object.keys(objectValue(params.values)).length > 200) {
    throw new Error(`Node '${node.label}' defines too many fields.`);
  }
  if ((node.type === "data.pick" || node.type === "data.remove") && (!Array.isArray(params.paths) || params.paths.length === 0 || params.paths.length > 200 || params.paths.some((path) => typeof path !== "string" || !path.trim()))) {
    throw new Error(`Node '${node.label}' requires one or more field paths.`);
  }
  if (node.type === "data.rename" && (typeof params.from !== "string" || !params.from.trim() || typeof params.to !== "string" || !params.to.trim())) {
    throw new Error(`Node '${node.label}' requires source and target paths.`);
  }
  if ((node.type === "data.template" || node.type === "data.parseJson" || node.type === "data.stringifyJson" || node.type === "text.transform" || node.type === "number.calculate" || node.type === "list.filter" || node.type === "list.sort" || node.type === "list.slice" || node.type === "date.now") && (typeof params.outputPath !== "string" || !params.outputPath.trim())) {
    throw new Error(`Node '${node.label}' requires an output path.`);
  }
  if (node.type === "logic.delay" && (!Number.isFinite(Number(params.delayMs)) || Number(params.delayMs) < 0 || Number(params.delayMs) > 60_000)) {
    throw new Error(`Node '${node.label}' requires a delay under 60 seconds.`);
  }
  if (node.type === "logic.condition" && (typeof params.path !== "string" || !params.path.trim())) {
