...
import { organizationMembers,roleBindings,roles,workspaceMembers,workspaces } from "@/server/infrastructure/db/schema";
...
export async function getSystemWorkspaceRole(roleName: string) {
  const [role] = await db
    .select()
    .from(roles)
    .where(and(eq(roles.scopeType, WORKSPACE_SCOPE), eq(roles.name, roleName), eq(roles.isSystem, true)))
    .limit(1);

  return role ?? null;
}

export async function addWorkspaceMember(input: { workspaceId: string; userId: string; roleName?: string; invitedBy: string }) {
  ...
  try {
    ...
    const [existingOrganizationMember] = await db
      ...
      .where(and(eq(organizationMembers.organizationId, workspace.organizationId), eq(organizationMembers.userId, userId)))
      .limit(1);
    ...
    await db.transaction(async (tx) => {
      if (existingOrganizationMember) {
        await tx.update(organizationMembers).set({ status: "active", updatedAt: new Date() }).where(eq(organizationMembers.id, existingOrganizationMember.id));
      } else {
      ...
      if (existingMember) {
        await tx.update(workspaceMembers).set({ status: "active", updatedAt: new Date() }).where(eq(workspaceMembers.id, existingMember.id));
      } else {
      ...
      if (existingBinding) {
        await tx.update(roleBindings).set({ roleId: role.id }).where(eq(roleBindings.id, existingBinding.id));
      } else {
    ...
    logger.error("Failed to add workspace member", { workspaceId, userId }, error as Error);
    throw error;
  }
}
