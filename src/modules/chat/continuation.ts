...
export async function claimAssistantContinuation(input: { conversationId: string; messageId: string; providerId: string | null; modelId: string }): Promise<AssistantContinuationClaim> {
  const [latestMessage] = await db.select().from(messages).where(eq(messages.conversationId, input.conversationId)).orderBy(desc(messages.createdAt), desc(messages.id)).limit(1);

  if (!latestMessage || latestMessage.id !== input.messageId || latestMessage.role !== "assistant") {
    const [requestedMessage] = await db
      .select({ id: messages.id })
      .from(messages)
      .where(and(eq(messages.id, input.messageId), eq(messages.conversationId, input.conversationId), eq(messages.role, "assistant")))
      .limit(1);
  ...
  const [claimedMessage] = await db
    ...
    .where(and(eq(messages.id, input.messageId), ne(messages.status, "streaming")))
    .returning();
  if (!claimedMessage) return { status: "already_streaming" };

  await db.delete(messageParts).where(and(eq(messageParts.messageId, claimedMessage.id), inArray(messageParts.type, ["suggestions", "impact"])));
  ...
  return {
    ...
    nextSortOrder: existingParts.reduce((highest, part) => Math.max(highest, part.sortOrder), -1) + 1,
    appendableTextPart,
  };
}
