...
export const allowedChatImageMimeTypes = Object.keys(imageTypes);
...
export function assertChatImageAccess(
...
export async function createChatImageAttachment(input: {
	...
	try {
		...
		await storage.delete(metadataObjectKey(attachmentId)).catch(() => undefined);
		throw error;
	}
}
