...
import { AttachmentDetection,maxOfficeXmlBytes,maxPdfPages,utf8Decoder } from "./attachments.chat-image-attachment";
import { decodeXmlEntities,limitExtractedText,normalizeExtractedText } from "./attachments.detect-attachment";
import { declaredZipUncompressedSize,extractDocxMarkdown,extractSharedStrings,extractXmlText,markdownTable,spreadsheetColumnIndex,zipEntryNumber } from "./attachments.markdown-table";

function extractWorksheetMarkdown(xml: string, sharedStrings: string[]) {
  const rows = new Map<number, Map<number, string>>();
  for (const match of xml.matchAll(/<(?:[a-z0-9_-]+:)?c\b([^>]*)>([\s\S]*?)<\/(?:[a-z0-9_-]+:)?c>/gi)) {
    ...
    const rawValue = body.match(/<(?:[a-z0-9_-]+:)?v(?:\s[^>]*)?>([\s\S]*?)<\/(?:[a-z0-9_-]+:)?v>/i)?.[1];
    const value = type === "s" && rawValue !== undefined ? (sharedStrings[Number.parseInt(rawValue, 10)] ?? rawValue) : type === "inlineStr" ? extractXmlText(body) : decodeXmlEntities(rawValue ?? extractXmlText(body));
...
export async function extractOfficeText(bytes: Uint8Array, textKind: Extract<AttachmentDetection["textKind"], "docx" | "pptx" | "xlsx">) {
  const zip = await JSZip.loadAsync(bytes);
  const entries = Object.values(zip.files)
    .filter((entry) => !entry.dir)
    .filter((entry) => {
      if (textKind === "docx") {
        return /^word\/(?:document|footnotes|endnotes|comments)\.xml$/i.test(entry.name) || /^word\/(?:header|footer)\d+\.xml$/i.test(entry.name);
      }
      if (textKind === "pptx") return /^ppt\/slides\/slide\d+\.xml$/i.test(entry.name);
  ...
  if (textKind === "docx") {
    ...
    const sharedStringsEntry = loadedEntries.find((entry) => /xl\/sharedStrings\.xml$/i.test(entry.name));
    const sharedStrings = sharedStringsEntry ? extractSharedStrings(sharedStringsEntry.xml) : [];
    markdown = loadedEntries
      .filter((entry) => /xl\/worksheets\/sheet\d+\.xml$/i.test(entry.name))
      .map((entry) => {
        const table = extractWorksheetMarkdown(entry.xml, sharedStrings);
        return table ? `## Sheet ${zipEntryNumber(entry.name)}\n\n${table}` : "";
      })
      .filter(Boolean)
      .join("\n\n");
  }

  return limitExtractedText(markdown, truncated ? "The document was partially read because it is large." : undefined, truncated);
}
...
export async function extractPdfMarkdown(bytes: Uint8Array) {
  ...
  try {
    ...
    return limitExtractedText(markdown, pagesTruncated ? `Only the first ${maxPdfPages} PDF pages were extracted.` : markdown ? undefined : "No readable text was found in this PDF; scanned pages may require OCR.", pagesTruncated);
  } finally {
