...
function toolResultText(content: string | Array<{ type: "text"; text: string }> | undefined) {
...
function anthropicMessages(request: AnthropicMessagesRequest): ModelMessage[] {
  ...
  for (const [messageIndex, message] of request.messages.entries()) {
    ...
    if (message.role === "assistant") {
      ...
      for (const block of message.content) {
        ...
        if (block.type !== "text" && block.type !== "tool_use") {
          throw invalidRequest(`Unsupported assistant content block '${block.type}'.`, `messages.${messageIndex}.content`);
        }
      }
    ...
    const userContent: Array<{ type: "text"; text: string } | { type: "file"; data: string | URL; mediaType: string }> = [];
    ...
    for (const [blockIndex, block] of message.content.entries()) {
      if (block.type === "text") userContent.push(block);
        ...
        userContent.push({
          type: "file",
          data: block.source.type === "url" ? new URL(block.source.url) : block.source.data,
          mediaType: block.source.type === "url" ? "image/*" : block.source.media_type,
        });
      } else if (block.type === "tool_result") {
        const toolName = toolNames.get(block.tool_use_id);
        if (!toolName) {
          throw invalidRequest(`No matching tool_use was found for '${block.tool_use_id}'.`, `messages.${messageIndex}.content.${blockIndex}.tool_use_id`);
        }
        toolResults.push({
          role: "tool",
          content: [
        ...
        throw invalidRequest(`Unsupported user content block '${block.type}'.`, `messages.${messageIndex}.content.${blockIndex}`);
      }
    }
...
function toolChoiceFrom(request: AnthropicMessagesRequest): ToolChoice<ToolSet> | undefined {
...
export function prepareAnthropicMessages(request: AnthropicMessagesRequest): PreparedProxyGeneration {
  ...
  const disableParallelToolUse = request.tool_choice && "disable_parallel_tool_use" in request.tool_choice ? request.tool_choice.disable_parallel_tool_use : undefined;
  return {
