...
async function getEncryptionKey(): Promise<CryptoKey> {
  ...
  cachedKey = await crypto.subtle.importKey("raw", keyBuffer, { name: ALGO }, false, ["encrypt", "decrypt"]);
  return cachedKey;
}
...
export async function decryptValue(encryptedJson: string): Promise<string> {
  ...
  if (payload.kid !== env.APP_ENCRYPTION_KEY_ID) {
    throw new Error(`Encryption key ID mismatch: expected ${env.APP_ENCRYPTION_KEY_ID}, got ${payload.kid}`);
  }
 ...
 */
export async function hashWithSalt(value: string): Promise<{ hash: string; salt: string }> {
