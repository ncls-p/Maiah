...
import { attachRequestId,AuthSession,logRouteCompleted,logRouteRejected,requestIdFrom,RouteHandlerOptions,routeLogData } from "./route-handler.route-handler-options";

/**
 * Wrap an async handler with admin session auth + error handling.
 * Requires the user to have the admin role.
 */
export async function handleAdminRoute(req: NextRequest, handler: (ctx: { session: AuthSession; request: NextRequest; requestId: string }) => Promise<Response>, opts?: RouteHandlerOptions): Promise<Response> {
  ...
  try {
    ...
    if (!(await isPlatformAdminSession(session))) {
      logRouteRejected(req, requestId, startedAt, "admin", 403, "not_platform_admin", session);
    ...
    return logRouteCompleted(req, requestId, startedAt, "admin", response, session);
  } catch (error) {
    ...
    logHandledError(opts?.logLabel ?? "Admin route handler error", routeLogData(req, requestId, startedAt, "admin", 500), error as Error);
