...
export const APP_SIDEBAR_OPEN_STORAGE_EVENT = "chat-unified-sidebar-open-change";
...
const LEGACY_WIDTH_STORAGE_KEYS = ["chat-unified-sidebar-width", "workspace-sidebar-width"] as const;

export function clampAppSidebarWidth(value: number) {
  return Math.min(MAX_APP_SIDEBAR_WIDTH, Math.max(MIN_APP_SIDEBAR_WIDTH, Math.round(value)));
}
...
export function getStoredAppSidebarWidth(): number {
  const stored = window.localStorage.getItem(APP_SIDEBAR_WIDTH_STORAGE_KEY) ?? LEGACY_WIDTH_STORAGE_KEYS.map((key) => window.localStorage.getItem(key)).find((value) => value !== null);
  const parsed = stored ? Number.parseInt(stored, 10) : DEFAULT_APP_SIDEBAR_WIDTH;
  return Number.isFinite(parsed) ? clampAppSidebarWidth(parsed) : DEFAULT_APP_SIDEBAR_WIDTH;
}

export function setStoredAppSidebarWidth(width: number) {
  window.localStorage.setItem(APP_SIDEBAR_WIDTH_STORAGE_KEY, String(clampAppSidebarWidth(width)));
