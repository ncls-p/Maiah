...
import { createLocalMessage,prepareAssistantMessageContinuation,toolNameMatches,type ChatCitation,type ChatMessage,type ChatStreamEvent,type PendingToolApproval } from "@/components/chat/chat-types";
import { streamAiSdkUIChat } from "@/hooks/ai-sdk-ui-chat-transport";

import { STREAM_DRAFT_EVENT,STREAM_DRAFT_WRITE_BATCH_MS,STREAM_RENDER_BATCH_MS,TOOL_CALL_PART_TYPE,applyStreamEvent,approvalsFromDraft,clearStoredChatStreamDraft,filterResolvedApprovals,getStoredChatStreamDraft,mergeStoredDraft,parseStreamEventText,removePendingApproval,storeChatStreamDraft,upsertPendingApproval,type StoredChatStreamDraft } from "@/hooks/use-chat-stream-events";
import { SubmitOptions,UseChatStreamOptions,appendErrorPart,compactErrorMessage } from "./use-chat-stream.compact-error-message";
...
export function useChatStream({ agentId, conversationId, workspaceId, canChat, onConversationCreated, onConversationTitle, onConversationsRefresh }: UseChatStreamOptions) {
  ...
  }, [conversationId, streamingMessageId, sending, reloadConversationMessages, onConversationsRefresh]);
