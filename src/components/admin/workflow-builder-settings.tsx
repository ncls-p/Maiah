...
export function WorkflowBuilderSettings() {
  ...
  const loadSettings = useCallback(
    async (signal?: AbortSignal) => {
      ...
      setLoadError(false);
      try {
        const res = await fetch(`/api/admin/workflow-builder?workspaceId=${workspaceId}`, { signal });
  ...
  const readyAgents = useMemo(() => state?.availableAgents.filter((agent) => agent.ready) ?? [], [state]);
  const selectedAgent = state?.availableAgents.find((agent) => agent.id === agentId);
  ...
  const badge = selectedAgent ? { label: t("statusReady"), tone: "success" as const } : agentId ? { label: t("statusUnavailable"), tone: "warning" as const } : { label: t("statusAutomatic"), tone: "muted" as const };

  return (
    <SettingsSection icon={WorkflowIcon} title={t("title")} description={t("description")} stagger="stagger-4" badge={<SettingsStatusBadge {...badge} />}>
      <div className="space-y-5">
        <SettingsDisabledNotice title={t("toolsTitle")} description={t("toolsDescription")} />

        {readyAgents.length === 0 ? (
          <div className="rounded-xl border border-dashed bg-background p-5 text-center">
            <BotIcon className="mx-auto size-6 text-muted-foreground" aria-hidden="true" />
          ...
          <div className="space-y-4 rounded-xl border bg-background p-4">
            <div className="space-y-2">
              ...
              <Select value={agentId ?? AUTOMATIC} onValueChange={(value) => setAgentId(value === AUTOMATIC ? null : value)} disabled={saving}>
                ...
                <SelectContent>
                  ...
                  {state.availableAgents.map((agent) => (
                    <SelectItem key={agent.id} value={agent.id} disabled={!agent.ready}>
                      {agent.name}
                      {agent.modelDisplayName ? ` · ${agent.modelDisplayName}` : ` · ${t("modelMissing")}`}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{agentId ? t("configuredHint") : t("automaticHint")}</p>
            </div>

            {selectedAgent ? (
              <div className="flex items-center gap-3 rounded-lg bg-muted/45 p-3">
                <ModelLogo label={selectedAgent.name} size="md" className="rounded-full" />
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium">{selectedAgent.name}</p>
                  <p className="truncate text-xs text-muted-foreground">{[selectedAgent.providerName, selectedAgent.modelDisplayName].filter(Boolean).join(" · ")}</p>
                </div>
        ...
        <div className="flex justify-end border-t border-border/60 pt-4">
          <Button onClick={() => void save()} disabled={saving || readyAgents.length === 0}>
