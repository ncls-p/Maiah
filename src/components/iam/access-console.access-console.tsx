...
import { AlertTriangleIcon,ArrowRightLeftIcon,BoxesIcon,Building2Icon,CheckIcon,CopyIcon,EllipsisIcon,FolderKanbanIcon,LockKeyholeIcon,PencilIcon,PlusIcon,RefreshCwIcon,SearchIcon,ShieldCheckIcon,ShieldIcon,Trash2Icon,UserPlusIcon,UsersIcon } from "lucide-react";
...
import { ConfirmRemovalButton,MutatingButton,ScopePath } from "./access-console.scope-path";
import { TeamCard } from "./access-console.team-card";

export function AccessConsole({ platformUsers, currentUserId }: { platformUsers?: PlatformAccessUser[]; currentUserId?: string }) {
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-4 px-0">
              <div className="flex flex-col gap-3 px-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                {selectedPeople.length > 0 && (canManageProjectAccess || canManageOrganizationAccess) ? (
                  <div className="flex flex-wrap items-center justify-end gap-2">
                    <span className="text-sm text-muted-foreground">{t("selectedCount", { count: selectedPeople.length })}</span>
                    <Button
                      type="button"
                      size="sm"
                      onClick={() => {
                    ...
                    <Button type="button" size="sm" variant="outline" onClick={() => void openMemberTransfer()}>
                    ...
                    >
                      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
                        ...
                        {memberTransferLoading ? (
                          ...
                          <Empty className="min-h-48 border">
                            <EmptyHeader>
                              ...
                              <EmptyDescription>{t("noMemberTransferDestinationDescription")}</EmptyDescription>
                            </EmptyHeader>
                          </Empty>
                        ) : memberTransferPreview ? (
                          <div className="flex flex-col gap-4">
                            <Alert variant={memberTransferPreview.blockers.length > 0 ? "destructive" : "default"}>
                              {memberTransferPreview.blockers.length > 0 ? <AlertTriangleIcon aria-hidden="true" /> : <CheckIcon aria-hidden="true" />}
                              <AlertTitle>
                              ...
                              <AlertDescription>
                                {memberTransferPreview.blockers.length > 0
                                  ...
                                  : t("memberTransferReadyDescription", {
                                      project: memberTransferPreview.destination.workspaceName,
                                      organization: memberTransferPreview.destination.organizationName,
                                    })}
                              </AlertDescription>
                            </Alert>
                            ...
                            <div className="grid gap-3 sm:grid-cols-2">
                              <div className="rounded-xl border p-4">
                                <div className="text-2xl font-semibold">{memberTransferPreview.changes.destinationAssignmentsAdded}</div>
                                <div className="text-sm text-muted-foreground">{t("destinationAccessAdded")}</div>
                              </div>
                              <div className="rounded-xl border p-4">
                                <div className="text-2xl font-semibold">{memberTransferPreview.changes.sourceAssignmentsRemoved}</div>
                                <div className="text-sm text-muted-foreground">{t("sourceAccessRemoved")}</div>
                              </div>
                              {memberTransferPreview.destination.crossOrganization ? (
                                <>
                                  <div className="rounded-xl border p-4">
                                    <div className="text-2xl font-semibold">{memberTransferPreview.changes.destinationMembershipsAdded}</div>
                                    <div className="text-sm text-muted-foreground">{t("organizationMembershipsAdded")}</div>
                                  </div>
                                  <div className="rounded-xl border p-4">
                                    <div className="text-2xl font-semibold">{memberTransferPreview.changes.sourceTeamMembershipsRemoved}</div>
                                    <div className="text-sm text-muted-foreground">{t("teamMembershipsRemoved")}</div>
                                  </div>
                                </>
                              ) : null}
                            </div>
                            ...
                            {memberTransferPreview.warnings.map((warning) => (
                              <Alert key={warning}>
                                ...
                                <AlertDescription>{t(`memberTransferWarnings.${warning}`)}</AlertDescription>
                              </Alert>
                            ))}
                          </div>
                        ) : (
                          <FieldGroup>
                            <Field>
                              <FieldLabel htmlFor="member-transfer-search">{t("destinationProject")}</FieldLabel>
                              <div className="relative">
                                <SearchIcon className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" />
                                <Input id="member-transfer-search" className="pl-9" value={memberTransferQuery} placeholder={t("searchProjects")} onChange={(event) => setMemberTransferQuery(event.target.value)} />
                              </div>
                              <Select
                              ...
                              >
                                <SelectTrigger className="w-full" aria-label={t("destinationProject")}>
                            ...
                            <Field>
                              ...
                              <div className="grid gap-3 sm:grid-cols-2">
                                {(["add", "move"] as const).map((mode) => (
                                  <button key={mode} type="button" className={`rounded-xl border p-4 text-left transition-colors ${memberTransferMode === mode ? "border-primary bg-primary/5" : "hover:bg-muted/50"}`} onClick={() => setMemberTransferMode(mode)}>
                                    <span className="font-medium">{t(mode === "add" ? "addToDestination" : "moveToDestination")}</span>
                                    <span className="mt-1 block text-sm text-muted-foreground">{t(mode === "add" ? "addToDestinationDescription" : "moveToDestinationDescription")}</span>
                                  </button>
                                ))}
                              </div>
                            </Field>
                            ...
                            <Field>
                              ...
                              <Select value={memberTransferRoleId} disabled={!selectedMemberTransferDestination} onValueChange={setMemberTransferRoleId}>
                                <SelectTrigger className="w-full" aria-label={t("destinationRole")}>
                              ...
                              <FieldDescription>{t("destinationRoleDescription")}</FieldDescription>
                            </Field>
                          </FieldGroup>
                        )}
                        ...
                        <DialogFooter>
                          {memberTransferPreview ? (
                            <>
                              <Button type="button" variant="outline" onClick={() => setMemberTransferPreview(null)}>
                                {t("back")}
                              </Button>
                              <Button type="button" disabled={memberTransferPreview.blockers.length > 0 || pendingAction === "executeMemberTransfer"} onClick={() => void confirmSelectedMemberTransfer()}>
                                {pendingAction === "executeMemberTransfer" ? <Spinner /> : <ArrowRightLeftIcon aria-hidden="true" />}
                                {t(memberTransferMode === "add" ? "confirmAddMembers" : "confirmMoveMembers")}
                              </Button>
                            </>
                          ) : memberTransferDestinations.length > 0 ? (
                            <Button type="button" disabled={!memberTransferTargetId || !memberTransferRoleId || pendingAction === "previewMemberTransfer"} onClick={() => void previewSelectedMemberTransfer()}>
                              {pendingAction === "previewMemberTransfer" ? <Spinner /> : <ShieldCheckIcon aria-hidden="true" />}
