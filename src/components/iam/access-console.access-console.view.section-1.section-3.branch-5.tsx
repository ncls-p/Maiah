...
export function AccessPeopleBranch5({ model }: { model: AccessConsoleViewModel }) {
  const { canManageMembers, canManageOrganizationAccess, canManageProjectAccess } = model;
  return (
    <CardAction className="col-start-1 row-start-3 row-span-1 mt-2 flex-wrap justify-self-start lg:col-start-2 lg:row-start-1 lg:row-span-2 lg:mt-0 lg:justify-self-end">
      ...
      {canManageProjectAccess || canManageOrganizationAccess ? <AccessPeopleTransferBranch1 model={model} /> : null}
    </CardAction>
  );
}
