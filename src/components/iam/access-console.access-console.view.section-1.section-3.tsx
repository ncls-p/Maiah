...
export function AccessMainSection3({ model }: { model: AccessConsoleViewModel }) {
  const { canManageMembers, canManageOrganizationAccess, canManageProjectAccess, people, peopleQuery, selectedPeople, setPeopleQuery, setVisiblePeopleCount, t, visiblePeople } = model;
  return (
    <TabsContent value="access" className="flex flex-col gap-4">
      <Card>
        <CardHeader className="grid-cols-1! lg:grid-cols-[1fr_auto]!">
          ...
          {canManageMembers || canManageProjectAccess || canManageOrganizationAccess ? <AccessPeopleBranch5 model={model} /> : null}
        </CardHeader>
        <CardContent className="flex flex-col gap-4 px-0">
          <div className="flex flex-col gap-3 px-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            {selectedPeople.length > 0 && (canManageProjectAccess || canManageOrganizationAccess) ? <AccessPeopleBranch4 model={model} /> : null}
          </div>

          {people.length === 0 ? <AccessPeopleBranch3 model={model} /> : <AccessPeopleBranch2 model={model} />}
          {people.length > visiblePeople.length ? <AccessPeopleBranch1 model={model} /> : null}
        </CardContent>
      </Card>
    </TabsContent>
  );
