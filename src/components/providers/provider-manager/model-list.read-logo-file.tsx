...
import type { DiscoveredModel,ProviderModel,ProviderModelUpdate,SafeProvider } from "./types";
  ...
  if (props.selectedProvider) {
    return (
      <section className="rounded-xl border bg-card">
        <ModelsHeader {...props} />
        <AdvancedSection label={t("manualModel")} hint={t("manualModelHint")} storageKey="advanced:provider-manual-model" className="m-4 mb-0 border-border/60 bg-muted/20">
...
function ModelsHeader({ selectedProvider, busy, onDiscoverModels }: ModelsPanelProps) {
  const t = useTranslations("providers.manager");
  return (
    <div className="flex flex-col gap-3 border-b px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">{t("modelsLoadedAutomatically")}</p>
      </div>
      <Button size="sm" variant="outline" disabled={busy} onClick={onDiscoverModels}>
...
function ManualModelForm({ manualModelId, manualModelName, busy, onCreateModel, onManualModelIdChange, onManualModelNameChange }: ModelsPanelProps) {
  const t = useTranslations("providers.manager");
  return (
    <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
      <div className="grid gap-1.5">
        ...
        <Input id="model-id" name="model-id" autoComplete="off" value={manualModelId} onChange={(e) => onManualModelIdChange(e.target.value)} placeholder="gpt-4o-mini…" className="font-mono text-sm" />
      </div>
      <div className="grid gap-1.5">
        <Label htmlFor="model-display-name" className="text-xs">
          {t("displayName")}
        </Label>
        <Input id="model-display-name" name="model-display-name" autoComplete="off" value={manualModelName} onChange={(e) => onManualModelNameChange(e.target.value)} placeholder="GPT-4o mini…" className="text-sm" />
      </div>
      <div className="flex items-end">
        <Button size="sm" disabled={busy || !manualModelId} onClick={() => onCreateModel()}>
