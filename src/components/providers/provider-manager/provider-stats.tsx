...
export function SystemStrip({ providers, models }: { providers: SafeProvider[]; models: ProviderModel[] }) {
  ...
  const totalModels = providers.reduce((sum, provider) => sum + models.filter((model) => model.providerId === provider.id).length, 0);
