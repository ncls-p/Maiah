...
function formatAgentPreview(preview: ManifestPreviewData, t: TranslateFn) {
  ...
  const tools = Number(preview.toolBindings ?? 0) + Number(preview.bundledMcp ?? 0) + Number(preview.bundledCustomTools ?? 0);
  const skills = Number(preview.skillBindings ?? 0) + Number(preview.bundledSkills ?? 0);
  const knowledgeBindings = Number(preview.knowledgeBindings ?? 0);

  if (preview.provider) {
    bullets.push(previewBullet(t("preview.agentProvider", { provider: String(preview.provider) })));
  }
  if (preview.model) {
    bullets.push(previewBullet(t("preview.agentModel", { model: String(preview.model) })));
  }
  if (tools > 0) {
  ...
  if (knowledgeBindings > 0) {
    bullets.push(previewBullet(t("preview.agentKnowledge", { count: knowledgeBindings })));
  }
...
function formatToolPreview(preview: ManifestPreviewData, t: TranslateFn) {
  ...
  if (preview.status) {
    bullets.push(previewBullet(t("preview.toolStatus", { status: String(preview.status) })));
  }
...
function formatMcpPreview(preview: ManifestPreviewData, t: TranslateFn) {
  ...
  if (preview.transport) {
    bullets.push(previewBullet(t("preview.mcpTransport", { transport: String(preview.transport) })));
  }
  if (toolCount > 0) {
...
const PREVIEW_FORMATTERS: Record<string, (preview: ManifestPreviewData, t: TranslateFn) => PreviewBullet[]> = {
...
export function formatManifestPreview(preview: ManifestPreviewData, t: TranslateFn): PreviewBullet[] {
  const formatter = PREVIEW_FORMATTERS[String(preview.type ?? "")];
  return formatter?.(preview, t) ?? [];
}

export function getItemTypeLabel(type: string, t: (key: string) => string): string {
  const key = type as "agent" | "skill" | "custom_tool" | "mcp_preset" | "prompt_template" | "tool_pack" | "workflow_template" | "knowledge_template" | "provider_preset";
  if (["agent", "skill", "custom_tool", "mcp_preset", "prompt_template", "tool_pack", "workflow_template", "knowledge_template", "provider_preset"].includes(key)) {
...
export function getVisibilityHint(visibility: string, t: (key: string) => string): string | undefined {
...
export function getToolSourceLabel(source: string, t: (key: string) => string): string {
...
export function formatMarketplaceDate(dateStr: string | null, locale: string, style: "short" | "long" = "short") {
  if (!dateStr) return "—";
