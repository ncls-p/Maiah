...
function DropdownMenuTrigger({
  ...
  const composedRef = React.useCallback(
    (node: HTMLElement | null) => {
      ...
      return () =>
        node.removeEventListener("pointerdown", preventPrimitivePointerToggle);
    },
    [ref],
  );
  return (
...
function DropdownMenuRadioItem({
  ...
  return (
    <DropdownMenuPrimitive.RadioItem
      ...
      className={cn(
        "relative flex min-h-9 cursor-default items-center gap-2 rounded-xl py-1.5 pr-2.5 pl-8 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    >
