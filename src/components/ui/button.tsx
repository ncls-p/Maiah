...
const buttonVariants = cva("group/button inline-flex shrink-0 items-center justify-center rounded-xl border border-transparent text-sm font-medium whitespace-nowrap outline-none select-none transition-[background-color,border-color,color,box-shadow,scale] duration-200 ease-out focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/18 disabled:pointer-events-none disabled:opacity-45 aria-invalid:border-destructive aria-invalid:ring-destructive/15 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4", {
  variants: {
    variant: {
      default: "bg-primary text-primary-foreground shadow-[0_1px_2px_color-mix(in_oklch,var(--primary)_22%,transparent)] hover:bg-primary/90",
      outline: "border-border/75 bg-card text-foreground shadow-none hover:border-border hover:bg-muted/75 aria-expanded:bg-muted",
      secondary: "border-transparent bg-secondary text-secondary-foreground shadow-none hover:bg-secondary/75 aria-expanded:bg-secondary",
      ghost: "text-foreground hover:bg-muted aria-expanded:bg-muted",
      destructive: "bg-destructive text-destructive-foreground shadow-[var(--control-shadow)] hover:bg-destructive/90 hover:shadow-[var(--control-shadow-hover)] focus-visible:ring-destructive/20",
...
function Button({
  ...
  return <Comp data-slot="button" data-variant={variant} data-size={size} className={cn(buttonVariants({ variant, size }), !isStatic && variant !== "link" && tapScale, className)} aria-label={ariaLabel} title={title ?? (typeof ariaLabel === "string" ? ariaLabel : undefined)} {...(!asChild ? { type: type ?? "button" } : {})} {...props} />;
}
