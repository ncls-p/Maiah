...
function Empty({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="empty" className={cn("flex w-full min-w-0 flex-1 flex-col items-center justify-center gap-5 rounded-2xl border border-dashed border-border/75 bg-muted/20 p-8 text-center text-balance shadow-none", className)} {...props} />;
}
...
const emptyMediaVariants = cva("mb-2 flex shrink-0 items-center justify-center [&_svg]:pointer-events-none [&_svg]:shrink-0", {
  variants: {
    variant: {
      ...
      icon: "flex size-11 shrink-0 items-center justify-center rounded-xl border border-primary/10 bg-accent/70 text-primary [&_svg:not([class*='size-'])]:size-5",
    },
  },
  defaultVariants: {
    variant: "default",
...
function EmptyTitle({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="empty-title" className={cn("font-heading text-base font-semibold tracking-[-0.02em]", className)} {...props} />;
}
