...
export function AdvancedSection({ open, onOpenChange, form, setForm, prefix, placeholder, showConnectionMode = true }: { open: boolean; onOpenChange: (o: boolean) => void; form: McpServerForm; setForm: (f: McpServerForm) => void; prefix: string; placeholder: string; showConnectionMode?: boolean }) {
  const t = useTranslations("mcp.serverManager");
  return (
    <Collapsible open={open} onOpenChange={onOpenChange} className="min-w-0 overflow-hidden rounded-lg border border-border/70 bg-muted/20">
      <CollapsibleTrigger asChild>
        <Button type="button" variant="ghost" className="flex w-full justify-between px-3 py-2 text-sm">
      ...
      <CollapsibleContent className="grid min-w-0 gap-4 border-t border-border/60 p-3">
        ...
        {showConnectionMode ? (
          <div className="grid gap-3 rounded-lg border border-border/60 bg-background/60 p-3">
            <div className={FIELD_STACK_CLASS}>
              <Label htmlFor={`${prefix}-transport`} help={t("connectionModeHelp")}>
                {t("connectionMode")}
              </Label>
              <Select value={form.transport} onValueChange={(value) => setForm({ ...form, transport: value, authMode: "none" })}>
            ...
            {form.transport === "stdio" ? (
              <>
                ...
                <div className={FIELD_STACK_CLASS}>
                  ...
                  <p className="text-xs text-muted-foreground">{t("oneArgumentPerLine")}</p>
                </div>
              </>
            ) : null}
          </div>
        ...
        <div className="flex min-w-0 items-center justify-between gap-4 rounded-lg border border-border/60 bg-background/60 p-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("requireApprovalDescription")}</p>
          </div>
          <Switch aria-label={t("requireApprovalAria")} checked={form.requireApproval} onCheckedChange={(checked) => setForm({ ...form, requireApproval: checked })} />
        </div>
        <div className={FIELD_STACK_CLASS}>
          <Label htmlFor={`${prefix}-headers`} help={t("httpHeadersHint")}>
            {t("httpHeaders")}
          </Label>
          <Textarea id={`${prefix}-headers`} autoComplete="off" value={form.headers} onChange={(e) => setForm({ ...form, headers: e.target.value })} placeholder="Authorization=Bearer sk-…" />
        ...
        <div className={FIELD_STACK_CLASS}>
          ...
          <Textarea id={`${prefix}-env`} autoComplete="off" value={form.env} onChange={(e) => setForm({ ...form, env: e.target.value })} placeholder="API_KEY=…" />
