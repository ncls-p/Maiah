...
export function useWorkflowRunDetail({ workspaceId, loadRuns }: { workspaceId: string; loadRuns: () => Promise<void> }) {
  ...
  useEffect(() => {
    if (!runDetailOpen || !runDetail || !["queued", "running"].includes(runDetail.status)) return;
    const interval = window.setInterval(async () => {
      try {
        ...
        setRunDetail(payload.run);
        if (!["queued", "running"].includes(payload.run.status)) await loadRuns();
      } catch { /* Keep the last visible snapshot; the user can retry. */ }
    }, 1_500);
    return () => window.clearInterval(interval);
  }, [loadRuns, runDetail, runDetailOpen, workspaceId]);
  return { loadRunDetail, runDetail, runDetailLoading, runDetailOpen, setRunDetail, setRunDetailOpen } as const;
}
