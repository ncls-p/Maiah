...
export function startReasoningPart(parts: ChatMessage["parts"]): ChatMessage["parts"] {
  const lastPart = parts.at(-1);
  if (lastPart?.type === "reasoning" && lastPart.state === "streaming") {
    return parts;
  }
  return [...parts, { type: "reasoning", content: "", state: "streaming" as const }];
}
...
const hasStringDelta: StreamEventValidator = (event) => typeof event.delta === "string";

const hasToolIdentity: StreamEventValidator = (event) => typeof event.toolCallId === "string" && typeof event.toolName === "string";
...
const STREAM_EVENT_VALIDATORS: Record<string, StreamEventValidator> = {
  ...
  tool_approval_required: (event) => typeof event.invocationId === "string" && typeof event.toolName === "string",
  tool_input_start: hasToolIdentity,
  tool_input_delta: (event) => typeof event.toolCallId === "string" && typeof event.delta === "string",
  tool_input_snapshot: (event) => hasToolIdentity(event) && typeof event.inputText === "string",
  ...
  suggestions: (event) => Array.isArray(event.suggestions) && event.suggestions.every((item) => typeof item === "string"),
  impact: (event) => typeof event.impact === "object" && event.impact !== null,
  citations: (event) => Array.isArray(event.citations) || Array.isArray(event.sources),
};
...
export function isChatStreamEvent(value: unknown): value is ChatStreamEvent {
  ...
  const validator = typeof event.type === "string" ? STREAM_EVENT_VALIDATORS[event.type] : undefined;
