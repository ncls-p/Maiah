...
export const MessageContent = memo(function MessageContent({
  ...
  const { approvalByPartIndex, standaloneApprovals } = useMemo(() => {
    ...
    const findUnmatchedApproval = (pendingApprovals: PendingToolApproval[], part: (typeof renderableParts)[number]): PendingToolApproval | undefined =>
      pendingApprovals.find((item) => !matchedApprovalIds.has(item.invocationId) && toolPartMatchesApproval(part, item));
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {standaloneApprovals.length > 0
        ? standaloneApprovals.map((approval, approvalIndex) => <PendingApprovalCard key={approval.invocationId} pendingApproval={approval} sequence={approvalIndex + 1} onApprove={onApproveTool} onReject={onRejectTool} />)
        : null}
        ...
        partGroups.map((group) => {
          ...
          return (
            <WorkPhase
              ...
              hasPendingApproval={group.parts.some(({ partIndex }) => approvalByPartIndex.has(partIndex))}
              messageStatus={message.status}
            >
