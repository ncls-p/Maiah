...
export function ToolApprovalBanner({
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-3 rounded-2xl border border-warning/35 bg-warning/10 px-4 py-3 text-sm shadow-sm animate-in-up">
      <div className="flex items-start gap-2">
        <ShieldAlertIcon
          className="mt-0.5 size-4 shrink-0 text-warning animate-pulse"
          aria-hidden="true"
        />
          ...
          <Collapsible open={open} onOpenChange={setOpen}>
            <CollapsibleTrigger asChild>
              <Button
                ...
                variant="ghost"
                size="sm"
                className="mt-2 h-8 px-2 transition-all duration-200 hover:bg-muted/50"
              >
                <ChevronDownIcon
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button
          ...
          size="sm"
          onClick={onApprove}
          className="transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-primary/10 active:translate-y-0"
        >
