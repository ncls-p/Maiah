...
import { CheckIcon,ChevronDownIcon,FolderIcon,FolderPlusIcon,MessageSquareIcon,MessageSquarePlusIcon,MoreHorizontalIcon,PanelLeftCloseIcon,PanelLeftOpenIcon,PencilIcon,PinIcon,PlusIcon,RefreshCwIcon,SearchIcon,SearchXIcon,Trash2Icon,XIcon } from "lucide-react";
...
export function ChatSidebar({ agents, conversations, conversationFolders, activeConversationId, loading, searchQuery = "", searchResults = [], searching = false, searchError = false, hasMoreSearchResults = false, loadingMoreSearchResults = false, onSearchQueryChange, onRetrySearch, onLoadMoreSearchResults, onSelectConversation, onNewConversation, onRenameConversation, onDeleteConversation, onCreateConversationFolder, onRenameConversationFolder, onDeleteConversationFolder, onToggleConversationPin, onReorderConversations, hasMoreConversations, loadingMoreConversations, onLoadMoreConversations, collapsed, onCollapsedChange, className, shell, workspaceId, readOnly = false, showWorkspaceNavigation = true, footerContent }: ChatSidebarProps) {
  ...
  return (
    <div className={cn("flex h-full min-h-0 flex-col bg-transparent text-sidebar-foreground", className)}>
      <SidebarHeader
        contextLabel={t("conversations")}
        action={
          !readOnly && onCollapsedChange ? (
            <Button type={BUTTON_TYPE} size="icon-sm" variant={GHOST_VARIANT} className="size-10 shrink-0 rounded-xl text-muted-foreground transition-[background-color,color,scale] hover:bg-sidebar-accent/70 hover:text-sidebar-foreground active:scale-[0.96]" aria-label={t("collapseSidebar")} title={t("collapseSidebar")} onClick={() => onCollapsedChange(true)}>
      ...
      <div className="animate-in-fade flex min-h-0 flex-1 flex-col motion-reduce:animate-none">
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="flex shrink-0 flex-col gap-2 px-3 pb-2 pt-3">
            <Button type={BUTTON_TYPE} onClick={onNewConversation} className="h-11 w-full justify-start gap-2.5 rounded-xl px-3.5 text-sm shadow-[0_8px_22px_-16px_color-mix(in_oklch,var(--primary)_70%,transparent)]" aria-label={t("newConversation")}>
            ...
            <div className="relative flex items-center">
              <SearchIcon className="pointer-events-none absolute left-3 size-3.5 shrink-0 text-muted-foreground" aria-hidden="true" />
              <Input type="search" name="conversation-search" autoComplete="off" aria-label={t("searchLabel")} placeholder={readOnly ? t("searchCompactPlaceholder") : t("searchPlaceholder")} value={searchQuery} onChange={(event) => onSearchQueryChange?.(event.target.value)} className="h-11 min-w-0 rounded-xl border-sidebar-border/60 bg-card/60 pl-9 pr-11 text-xs shadow-none" />
              {searchActive ? (
                <Button type={BUTTON_TYPE} size="icon-sm" variant={GHOST_VARIANT} className="absolute right-0.5 size-10 shrink-0 rounded-[10px]" aria-label={t("clearSearch")} onClick={() => onSearchQueryChange?.("")}>
            ...
            <p className="sr-only" aria-live="polite">
              {searchActive && !searching && !searchError ? t("searchResultCount", { count: searchResults.length }) : null}
            </p>
            ...
            {!readOnly && creatingFolder ? (
              <div className="flex items-center gap-1 rounded-xl border border-sidebar-border/60 bg-background p-1">
                <Input
                  ...
                  autoFocus
                />
                <Button type={BUTTON_TYPE} size="icon-sm" variant={GHOST_VARIANT} aria-label={t("createFolder")} className="size-10 shrink-0 rounded-xl" onClick={saveNewFolder}>
                  <CheckIcon className="size-3" aria-hidden="true" />
                </Button>
                <Button type={BUTTON_TYPE} size="icon-sm" variant={GHOST_VARIANT} aria-label={t("cancelFolderCreation")} className="size-10 shrink-0 rounded-xl" onClick={() => setCreatingFolder(false)}>
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-1.5 overflow-y-auto px-3 pb-3 pt-1">
            {!searchActive ? (
              <div className="flex min-h-10 items-center justify-between gap-2 px-2">
                <span className="min-w-0 truncate font-mono text-[9px] font-medium uppercase tracking-[0.16em] text-muted-foreground/70">{t("recent")}</span>
            ...
            <div className="flex min-h-0 flex-col gap-1">
              {searchActive ? (
                searching && searchResults.length === 0 ? (
                  ...
                  <Empty className="border-0 bg-transparent px-2 py-10">
                    <EmptyHeader>
                      ...
                      <EmptyTitle className="text-sm font-medium">{t("searchErrorTitle")}</EmptyTitle>
                      <EmptyDescription className="text-xs text-muted-foreground/60">{t("searchErrorDescription")}</EmptyDescription>
                    </EmptyHeader>
                    {onRetrySearch ? (
                      <Button type={BUTTON_TYPE} variant="outline" size="sm" className="min-h-10 rounded-xl" onClick={onRetrySearch}>
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="px-2 pb-1 text-[11px] font-medium text-muted-foreground">{t("searchResultCount", { count: searchResults.length })}</div>
                    {searchResults.map((conversation) => renderConversation(conversation, { searchResult: true }))}
                    {searchError && onRetrySearch ? (
                      <Button type={BUTTON_TYPE} variant="ghost" size="sm" className="min-h-10 rounded-xl text-xs text-muted-foreground" onClick={onRetrySearch}>
                        <RefreshCwIcon data-icon="inline-start" aria-hidden="true" />
                        {t("retrySearch")}
                      </Button>
                    ) : null}
                    {hasMoreSearchResults && onLoadMoreSearchResults ? (
                      <Button type={BUTTON_TYPE} variant={GHOST_VARIANT} size="sm" className="mt-1 min-h-10 rounded-xl text-xs text-muted-foreground" disabled={loadingMoreSearchResults} onClick={onLoadMoreSearchResults}>
                ...
                <div className="pt-2">
                  <Empty className="border-0 bg-transparent px-2 py-10">
                    <EmptyHeader>
                      ...
                      <EmptyDescription className="text-xs text-muted-foreground/60">{t("emptyDescription")}</EmptyDescription>
                    </EmptyHeader>
                  </Empty>
                </div>
              ) : (
                <div className="flex flex-col gap-3">
                      ...
                      <section key={folder.id} className="flex flex-col gap-px">
                        ...
                        >
                          ...
                          {isEditingFolder ? (
                            ...
                            <button type={BUTTON_TYPE} className="flex min-h-10 min-w-0 flex-1 items-center gap-1 rounded-lg px-1 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40" aria-expanded={open} onClick={() => setFolderOpen(folder.id, !open)}>
                              <ChevronDownIcon className={cn("size-3 shrink-0 transition-transform", !open && "-rotate-90")} aria-hidden="true" />
                              <span className="truncate font-medium">{folder.name}</span>
                              <span className="text-muted-foreground/50">{folderConversations.length}</span>
                            </button>
                          )}
                          {!readOnly ? (
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button type={BUTTON_TYPE} size="icon-sm" variant={GHOST_VARIANT} className="size-10 rounded-xl transition-[background-color,opacity] md:opacity-0 md:group-hover/folder:opacity-100 md:group-focus-within/folder:opacity-100 data-[state=open]:opacity-100" aria-label={t("folderActions")}>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem variant="destructive" onSelect={() => onDeleteConversationFolder?.(folder.id)} className="min-h-10 gap-2">
                  ...
                  >
                    {topLevelConversations.length > 0 ? <>{topLevelConversations.map((conversation) => renderConversation(conversation))}</> : folderSections.length === 0 ? <div className="rounded-lg border border-dashed px-3 py-2 text-xs text-muted-foreground/60">{t("dropChatsHere")}</div> : null}
                  </section>

                  {hasMoreConversations && onLoadMoreConversations ? (
                    <Button type={BUTTON_TYPE} variant={GHOST_VARIANT} size="sm" className="mt-2 min-h-10 rounded-xl text-xs text-muted-foreground" disabled={loadingMoreConversations} onClick={onLoadMoreConversations}>
          ...
          {showWorkspaceNavigation && navGroups.length > 0 ? <ChatAppNavigation groups={navGroups} /> : null}
        </div>
      </div>
