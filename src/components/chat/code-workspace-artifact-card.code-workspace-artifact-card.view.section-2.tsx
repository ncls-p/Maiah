import { Maximize2Icon,RefreshCcwIcon,SaveIcon,Trash2Icon } from "lucide-react";
import type React from "react";

import { MAX_CODE_WIDTH,MAX_FILES_WIDTH,MIN_CODE_WIDTH,MIN_FILES_WIDTH,resizeCodeWorkspacePane } from "@/components/chat/code-workspace-layout";
import { CodeWorkspacePreviewFrame } from "@/components/chat/code-workspace-preview-frame";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { BUTTON_TYPE,COMPACT_ICON_CLASS,CodeWorkspaceResizeHandle,GHOST_VARIANT,OUTLINE_VARIANT } from "./code-workspace-artifact-card.button-type";
import type { CodeWorkspaceArtifactCardViewModel } from "./code-workspace-artifact-card.code-workspace-artifact-card.view";
import { CodeWorkspaceEditor,CodeWorkspaceFileTree } from "./code-workspace-artifact-card.code-workspace-file-tree";
export function CodeWorkspaceArtifactCardSection2({ model }: { model: CodeWorkspaceArtifactCardViewModel }) {
  const { content, currentArtifact, error, fileTree, loadingFile, paneId, saveSelectedFile, savingFile, selectedFile, selectedPath, setContent, setDeletePath, setFileReloadKey, setFullscreenPane, setSelectedPath, t, updateWorkspaceLayout, variant, visiblePanes, workspaceGridTemplate, workspaceLayout } = model;
  return (
    <div
