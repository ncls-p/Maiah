...
export function formatToolName(toolName: string | undefined) {
  ...
  return withoutPrefix.replace(/__+/g, " ").replace(/_/g, " ").replace(/\s+/g, " ").trim();
}

export function summarizeToolBody(toolName: string | undefined, body: unknown, isCall: boolean) {
  ...
  if (Array.isArray(body)) return `Returned ${body.length} item${body.length === 1 ? "" : "s"}.`;
  if (typeof body === "object") {
    ...
    return keys.length > 0 ? `Returned ${keys.slice(0, 3).join(", ")}${keys.length > 3 ? "…" : ""}.` : "The tool finished.";
  }
...
export function knowledgeSearchResultsFromUnknown(value: unknown): KnowledgeSearchResult[] | null {
  ...
  if (record.kind !== "knowledge_search_results" || !Array.isArray(record.results)) {
    return null;
  }
  return record.results.filter((result): result is KnowledgeSearchResult => {
    ...
    return typeof row.chunkId === "string" && typeof row.documentId === "string" && typeof row.documentTitle === "string" && typeof row.content === "string" && typeof row.knowledgeBaseName === "string" && typeof row.score === "number";
  });
}
...
export function isGeneratedImageOutput(value: unknown): value is GeneratedImageOutput {
  ...
  const record = value as Record<string, unknown>;
  return record.kind === "generated_image" && isChatImageAttachmentOutput(record.attachment) && typeof record.prompt === "string" && typeof record.model === "string";
}

export function isHtmlArtifactOutput(value: unknown): value is HtmlArtifactOutput {
  if (typeof value !== "object" || value === null) return false;
  const record = value as Record<string, unknown>;
...
export function isGitHubPublishOutput(value: unknown): value is GitHubPublishOutput {
  if (typeof value !== "object" || value === null) return false;
  const record = value as Record<string, unknown>;
  return record.kind === "github_publish_result" && typeof record.repository === "string" && typeof record.targetBranch === "string" && typeof record.commitSha === "string";
}
...
export function chatTodoListFromToolPart(part: ChatMessagePart): ChatTodoList | null {
