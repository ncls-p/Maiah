...
import type { ChatFileAttachment, ChatImageAttachment } from "@/components/chat/chat-types";
...
import { GithubIcon, formatBytes } from "./code-workspace-artifact-card.button-type";
  ...
  return (
    <div className="w-fit max-w-full overflow-hidden rounded-2xl bg-card text-xs shadow-[var(--surface-shadow)]">
      <div className="flex min-h-12 items-center gap-2.5 border-b border-border/40 px-2.5 py-1.5">
        ...
        <GithubIcon className="size-4 shrink-0 text-muted-foreground" aria-hidden="true" />
        <div className="min-w-0">
          <p className="font-medium text-foreground">{result.message}</p>
          <p className="truncate text-[11px] text-muted-foreground">
            {result.repository} · {result.mode === "pull_request" ? "PR" : t("directPush")} · {result.targetBranch}
          </p>
        </div>
      </div>
      <div className="flex flex-wrap items-center gap-2 px-3 py-2 text-[11px] text-muted-foreground">
        ...
        {result.pullRequestUrl ? (
          <a href={result.pullRequestUrl} target="_blank" rel="noreferrer" className="font-medium text-primary underline underline-offset-2">
            {t("openPr")}
          </a>
        ) : null}
      </div>
...
export function ChatImageAttachmentCard({ attachment }: { attachment: ChatImageAttachment }) {
  return (
    <Attachment orientation="vertical" className="w-[min(24rem,80vw)]">
      ...
      <AttachmentContent>
        ...
        <AttachmentDescription>
          {attachment.mimeType.replace("image/", "").toUpperCase()} · {formatBytes(attachment.size)}
        </AttachmentDescription>
      </AttachmentContent>
    </Attachment>
  );
...
export function ChatFileAttachmentCard({ attachment }: { attachment: ChatFileAttachment }) {
  ...
  const readLabel = attachment.extractionStatus === "unreadable" ? tComposer("storedSafely", { size: formatBytes(attachment.size) }) : attachment.extractionStatus === "truncated" ? tComposer("partiallyRead") : tComposer("readable");
    ...
    <>
      <Attachment className="max-w-[min(28rem,84vw)]">
        ...
        <AttachmentContent>
          ...
          <AttachmentDescription>
            {fileSummary}
            {attachment.extractedTextChars > 0 ? ` · ${t("extractedChars", { count: attachment.extractedTextChars })}` : ""}
          </AttachmentDescription>
        </AttachmentContent>
        <AttachmentActions>
          {canPreview ? (
            <AttachmentAction type="button" variant="outline" size="sm" onClick={preview.openPreview}>
      ...
      <FilePreviewDialog open={preview.previewOpen} onOpenChange={preview.setPreviewOpen} fileName={attachment.fileName} url={attachment.url} mimeType={attachment.mimeType} subtitle={`${fileSummary} · ` + t("extractedChars", { count: attachment.extractedTextChars })} previewText={preview.previewText} previewError={preview.previewError} loadingPreview={preview.loadingPreview} />
    </>
  );
}
