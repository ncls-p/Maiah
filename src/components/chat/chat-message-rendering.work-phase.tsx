  ...
  const statusLabel =
    (() => {
      const knowledgeResults = parts.flatMap(({ part }) => {
        ...
        const parsed = parseToolPart(part.content);
        return parsed.toolName === "search_knowledge" ? (knowledgeSearchResultsFromUnknown(parsed.output) ?? []) : [];
      });
      const knowledgeOnly = parts.every(({ part }) => {
        if (part.type === "reasoning") return true;
        const toolName = parseToolPart(part.content).toolName;
        return toolName === "search_knowledge" || toolName === "read_knowledge_context";
      });
      ...
      <div className="flex min-h-12 items-center gap-2.5 border-b border-border/45 px-3 py-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 items-center gap-2">
            <span className={cn("shrink-0 font-medium tracking-[-0.01em] text-foreground", visualState === "pending" && "t-shimmer")} data-text={visualState === "pending" ? statusLabel : undefined} aria-live="polite">
              {statusLabel}
            </span>
            <span className="truncate text-[10px] text-muted-foreground/70">{t("workPhaseSteps", { count: parts.length })}</span>
            {visualState === "pending" ? (
          ...
          <div className="flex min-w-0 items-center gap-1 overflow-hidden text-[11px] text-muted-foreground">
            ...
            {hiddenActivityCount > 0 ? <span className="shrink-0 text-muted-foreground/70">{t("workPhaseMore", { count: hiddenActivityCount })}</span> : null}
          </div>
        </div>
