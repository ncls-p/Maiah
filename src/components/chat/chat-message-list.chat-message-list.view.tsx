...
import { canContinueAssistantMessage,textFromMessage } from "@/components/chat/chat-types";
...
import { BUTTON_TYPE,EMPTY_PENDING_APPROVALS,LOAD_MORE_MESSAGES,MessageVisibilityPersistence,OUTLINE_VARIANT,SavedMessageAnchorRestorer } from "./chat-message-list.initial-visible-messages";
...
export type ChatMessageListViewModel = Extract<ReturnType<typeof useChatMessageListController>, { kind: "ready" }>;
export function ChatMessageListView({ model }: { model: ChatMessageListViewModel }) {
  const { bottomRef, contentRef, conversationId, editingContent, editingMessageId, hiddenMessageCount, lastAssistantMessageId, lastMessageId, locale, messageIndexById, messages, onApproveTool, onContinueAssistant, onDeleteMessage, onEditMessage, onJumpLatest, onRegenerateAssistant, onRejectTool, onResendMessage, onSuggestionClick, pendingApprovals, precedingUserByMessageId, savingMessageId, sending, setEditingContent, setEditingMessageId, setSavingMessageId, setVisibleMessageCount, t, userMessageShortcuts, viewportClassName, viewportRef, visibleMessages, workspaceArtifactDisplay, workspaceId } = model;
  return (
