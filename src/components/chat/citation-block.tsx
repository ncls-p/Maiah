...
export function CitationBlock({ citations, className }: CitationBlockProps) {
  ...
  return (
    <div
      className={cn(
        "rounded-xl border border-border/70 bg-muted/30 px-3 py-2.5 transition-all duration-200 hover:shadow-sm",
        className,
      )}
    >
      ...
      <ul className="flex flex-col gap-2">
        {citations.map((citation) => (
          <li
            ...
            className="rounded-lg border border-border/60 bg-background/80 px-3 py-2.5 text-xs transition-all duration-200 hover:border-primary/20 hover:shadow-sm"
          >
