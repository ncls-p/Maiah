...
export function TemporaryConversationButton({
  onSelect,
  tooltipSide = "bottom",
}: {
  onSelect: (ttlMinutes: number) => void;
    ...
    <DropdownMenu>
      <Tooltip>
        <TooltipTrigger asChild>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              size="icon"
              variant="ghost"
              className="size-11 shrink-0 rounded-xl border border-sidebar-border/60 bg-card/45"
              aria-label={sidebarT("newTemporaryConversation")}
            >
        ...
        <TooltipContent side={tooltipSide}>
          {sidebarT("newTemporaryConversation")}
        </TooltipContent>
      </Tooltip>
