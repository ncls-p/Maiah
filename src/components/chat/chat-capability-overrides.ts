...
export function readChatCapabilityOverrides(
  agentId: string | null,
  ...
  try {
    ...
    return {
      disabledTools: Array.isArray(value.disabledTools)
        ? value.disabledTools.filter(
            (tool): tool is { source: ChatToolSource; id: string } =>
              Boolean(
                tool &&
                ["builtin", "mcp", "custom"].includes(tool.source) &&
                typeof tool.id === "string",
              ),
          )
        : [],
      disabledSkillIds: Array.isArray(value.disabledSkillIds)
      ...
      enabledSkillIds: Array.isArray(value.enabledSkillIds)
        ...
        : [],
      enabledKnowledgeIds: Array.isArray(value.enabledKnowledgeIds)
        ? value.enabledKnowledgeIds.filter(
            (id): id is string => typeof id === "string",
          )
        : [],
    };
...
export function writeChatCapabilityOverrides(
  agentId: string,
  conversationId: string | null,
...
export function migrateDraftCapabilityOverrides(
  agentId: string,
