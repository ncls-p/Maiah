...
import {
  Capability,
  CapabilityGroup,
  EnabledKnowledgeSummary,
  EnabledSkillSummary,
  EnabledToolSummary,
  EnabledToolsPayload,
  isCapabilityActive,
  toolKey,
} from "./chat-tools-menu.enabled-tool-summary";
import {
...
export function ChatToolsMenu({
  ...
  const [categoryFilter, setCategoryFilter] =
    useState<ChatCapabilityCategory>("all");
  ...
  const capabilities = useMemo<Capability[]>(
    () => [
      ...
      ...knowledge.map((item) => ({
        ...
        name: item.name,
        description: item.description || t("toolsMenu.knowledgeReady"),
  ...
  const visibleCapabilities = capabilities.filter(
    (capability) =>
      (categoryFilter === "all" || capability.category === categoryFilter) &&
      (!normalizedSearch ||
  ...
  function setCapabilityActive(capability: Capability, active: boolean) {
    if (capability.source === "knowledge") {
      persistOverrides({
        ...overrides,
        enabledKnowledgeIds: active
          ? Array.from(
              new Set([...overrides.enabledKnowledgeIds, capability.id]),
            )
          : overrides.enabledKnowledgeIds.filter((id) => id !== capability.id),
      });
      return;
    }
        ...
        ...(capability.attached
          ? {
              disabledSkillIds: active
                ? overrides.disabledSkillIds.filter(
                    (id) => id !== capability.id,
                  )
                : Array.from(
                    new Set([...overrides.disabledSkillIds, capability.id]),
                  ),
            }
          : {
              enabledSkillIds: active
                ? Array.from(
                    new Set([...overrides.enabledSkillIds, capability.id]),
                  )
                : overrides.enabledSkillIds.filter(
                    (id) => id !== capability.id,
                  ),
            }),
      });
    ...
    for (const capability of group.capabilities) {
      ...
      if (capability.source === "knowledge") {
        next = {
          ...next,
          enabledKnowledgeIds: active
            ? Array.from(new Set([...next.enabledKnowledgeIds, capability.id]))
            : next.enabledKnowledgeIds.filter((id) => id !== capability.id),
        };
        continue;
      }
      if (capability.source === "skill") {
        next = capability.attached
          ? {
              ...next,
              disabledSkillIds: active
                ? next.disabledSkillIds.filter((id) => id !== capability.id)
                : Array.from(
                    new Set([...next.disabledSkillIds, capability.id]),
                  ),
            }
          : {
              ...next,
              enabledSkillIds: active
                ? Array.from(new Set([...next.enabledSkillIds, capability.id]))
                : next.enabledSkillIds.filter((id) => id !== capability.id),
            };
      } else {
        next = capability.attached
          ? {
  ...
  return (
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      ...
      <DropdownMenuContent
        ...
        align="start"
        sideOffset={12}
        className="flex h-[min(42rem,calc(100dvh-1rem))] w-[min(42rem,calc(100vw-1rem))] flex-col overflow-hidden rounded-[1.375rem] p-0"
      >
        <div className="flex shrink-0 items-start justify-between gap-4 px-4 pb-3 pt-4">
          ...
          <div className="flex shrink-0 gap-1">
            ...
            {overrides.disabledTools.length +
              overrides.disabledSkillIds.length +
              overrides.enabledTools.length +
              overrides.enabledSkillIds.length +
              overrides.enabledKnowledgeIds.length >
            0 ? (
              <Button
                type="button"
                variant="ghost"
        ...
        <div className="grid min-h-0 min-w-0 flex-1 grid-rows-[auto_minmax(0,1fr)] overflow-hidden border-y border-border/55 sm:grid-cols-[8.5rem_minmax(0,1fr)] sm:grid-rows-1">
          <ChatCapabilitySidebar
            ...
            label={t("toolsMenu.groups.all")}
            groupLabel={groupLabel}
          />
          <div
            data-slot="chat-capability-results"
            className="min-h-0 min-w-0 overflow-x-hidden overflow-y-auto overscroll-contain px-2 py-2 [scrollbar-gutter:stable]"
          >
            {loading ? (
                        ...
                        {group.capabilities.map((capability) => {
                          ...
                          return (
                            <label
                              key={capability.key}
                              className={cn(
                                "flex min-h-12 min-w-0 cursor-pointer items-center gap-2 overflow-hidden rounded-xl px-2 py-2 transition-[background-color,color,opacity] hover:bg-muted/65 sm:gap-3",
                                !active && "opacity-55",
                              )}
                            >
                              ...
                              <span className="min-w-0 flex-1 overflow-hidden">
                                ...
                                <small
                                  className="mt-0.5 block truncate text-[0.65rem] text-muted-foreground"
        ...
        <ChatCapabilityFooter
          agentId={agent.id}
          chatOnly={t("toolsMenu.chatOnly")}
          customize={t("toolsMenu.customize")}
        />
      </DropdownMenuContent>
    </DropdownMenu>
  );
