...
export function LiveToolInputCard({ toolName, inputText, sandboxInput, embedded = false }: { toolName: string; inputText: string; sandboxInput?: CodeSandboxInputPreview | null; embedded?: boolean }) {
  ...
  return (
    <div className={cn("overflow-hidden rounded-2xl bg-primary/[0.055] text-xs shadow-[inset_0_0_0_1px_color-mix(in_oklch,var(--primary)_18%,transparent),0_14px_28px_-24px_color-mix(in_oklch,var(--primary)_55%,transparent)]", embedded && "rounded-xl border border-border/55 bg-background/45 shadow-none")}>
      <div className="flex items-center gap-2.5 border-b border-border/40 px-2.5 py-1.5">
        ...
        <span className="streaming-thinking__dots mr-2 text-primary" aria-hidden="true">
          <span />
          <span />
          <span />
        </span>
      ...
      {sandboxInput ? (
        <div className="flex flex-wrap gap-2 border-b border-border/40 px-3 py-2 text-[10px] text-muted-foreground">
          {sandboxInput.files.length > 0 ? <span>{t("inputFiles", { count: sandboxInput.files.length })}</span> : null}
          {sandboxInput.attachments.length > 0 ? <span>{t("attachments", { count: sandboxInput.attachments.length })}</span> : null}
        </div>
      ) : null}
      <pre className="max-h-72 overflow-auto bg-muted/20 p-3 font-mono text-[11px] leading-4 text-muted-foreground whitespace-pre-wrap">{displayText || t("waitingInput")}</pre>
    </div>
  );
}
