...
export const maxRequestBytes = Number(
  process.env.SANDBOX_MAX_REQUEST_BYTES ?? 72_000_000,
);
export const maxCodeChars = Number(
  process.env.SANDBOX_MAX_CODE_CHARS ?? 100_000,
);
...
export const maxInputTotalBytes = Number(
  process.env.SANDBOX_MAX_INPUT_TOTAL_BYTES ?? 50_000_000,
);
export const maxInputFiles = Number(process.env.SANDBOX_MAX_INPUT_FILES ?? 500);
export const maxInlineStdinChars = Number(
  process.env.SANDBOX_MAX_INLINE_STDIN_CHARS ?? 100_000,
);
export const maxStdoutBytes = Number(
  process.env.SANDBOX_MAX_STDOUT_BYTES ?? 64_000,
);
export const maxStdoutFileBytes = Number(
  process.env.SANDBOX_MAX_STDOUT_FILE_BYTES ?? 1_500_000,
);
export const maxStderrBytes = Number(
  process.env.SANDBOX_MAX_STDERR_BYTES ?? 64_000,
);
export const maxFilePreviewBytes = Number(
  process.env.SANDBOX_MAX_FILE_PREVIEW_BYTES ?? 16_000,
);
export const maxCollectedFiles = Number(
  process.env.SANDBOX_MAX_COLLECTED_FILES ?? 500,
);
export const maxCollectedFileBytes = Number(
  process.env.SANDBOX_MAX_COLLECTED_FILE_BYTES ?? 50_000_000,
);
export const maxDownloadFileBytes = Number(
  process.env.SANDBOX_MAX_DOWNLOAD_FILE_BYTES ?? 50_000_000,
);
export const maxDownloadTotalBytes = Number(
  process.env.SANDBOX_MAX_DOWNLOAD_TOTAL_BYTES ?? 50_000_000,
);
